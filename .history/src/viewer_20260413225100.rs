                    ...
                    let result = (|| -> Option<LoadResult> {
                        ...
                        let img = if req.entry_name.to_lowercase().ends_with(".webp") {
                            match webp::Decoder::new(&bytes).decode() {
                                Ok(webp_img) => {
                                    // webp形式をRGBA8に変換
                                    match webp_img.to_image() {
                                        Some(dynamic) => dynamic.to_rgba8(),
                                        None => {
                                            eprintln!("Failed to convert webp to image: {}", req.entry_name);
                                            return None;
                                        }
                                    }
                                }
                                Err(e) => {
                                    eprintln!("Failed to decode webp {}: {}", req.entry_name, e);
                                    return None;
                                }
                            }
                        } else {
