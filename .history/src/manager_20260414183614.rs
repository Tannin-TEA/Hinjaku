...
pub struct Manager {
    ...
    list_rx: Option<Receiver<Result<(PathBuf, Vec<archive::ImageEntry>), String>>>,
...
impl Manager {
            ...
            if let Ok(res) = rx.try_recv() {
                ...
                match res {
                    Ok((path, entries)) => {
                        ...
                        // ここでエントリ名を更新し、呼び出し側に通知するために空の failures を返すか
                        // 本来は viewer 側で entries を再構築する必要があります
                    }
    ...
    pub fn open_path(&mut self, path: PathBuf, _config: &Config) {
        ...
        std::thread::spawn(move || {
            let res = archive::list_images(&bp_clone)
        ...
        if let Some(name) = start_name {
            self.entries = vec![name]; // ロード中の表示用
        }
    }
    ...
    pub fn get_current_full_path(&self) -> Option<String> {
        ...
        let entry = self.entries.get(self.target_index)?; // currentよりtarget_indexの方が確実
...
impl NavTree {
    ...
    pub fn get_siblings(&mut self, path: &Path) -> Vec<PathBuf> {
        if let Some(p) = path.parent() {
            ...
            archive::get_roots() // 親がなければドライブ一覧を兄弟とする
        }
    }
    ...
    pub fn move_selection(&mut self, delta: isize) {
        if self.selected.is_none() {
            if let Some(first_root) = archive::get_roots().first() {
                self.selected = Some(archive::clean_path(first_root));
            }
            return;
        }

        if let Some(sel) = self.selected.clone() {
            let sel = archive::clean_path(&sel);
            ...
            if let Some(pos) = siblings.iter().position(|p| p == &sel) {
                ...
                self.selected = Some(archive::clean_path(&siblings[new_pos]));
            }
        }
    }
        ...
        if let Some(sel) = self.selected.clone() {
            ...
            if kind == archive::ArchiveKind::Plain {
                ...
                if let Some(first) = children.first() {
                    self.selected = Some(archive::clean_path(first));
                }
            }
        }
    }
    ...
    pub fn collapse_or_up(&mut self) {
        if let Some(sel) = self.selected.clone() {
            ...
            if self.expanded.contains(&sel) {
                ...
                // すでに閉じている（またはファイル）なら親へ戻り、同時にその戻り元を閉じる
                let p = archive::clean_path(parent);
                if p != sel {
                    self.selected = Some(p.clone());
                    self.expanded.remove(&sel); // 以前いた場所（子）を閉じる
                }
            }
        }
    }
