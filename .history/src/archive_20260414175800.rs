...
pub fn join_entry_path(archive_path: &Path, entry_name: &str) -> String {
    // archive_path がファイル(zip)であっても Path::join は機能する。
    // 内部パス entry_name の / を Windows の \ に置換して返す。
    archive_path.join(entry_name).to_string_lossy().replace('/', "\\")
}
