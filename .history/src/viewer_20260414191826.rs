...
pub struct App {
    ...
    /// 操作をロック解除する予定時刻 (f64::MAX はロード待ちによる永続ロック)
    nav_lock_until: f64,
...
impl App {
    ...
    fn is_nav_locked(&self, ctx: &egui::Context) -> bool {
        let now = ctx.input(|i| i.time);
        if now < self.nav_lock_until {
            // ウェイト中なら、タイマー終了時にUIが更新されるよう再描画を予約
            if self.nav_lock_until != f64::MAX {
                ctx.request_repaint_after(std::time::Duration::from_secs_f64(self.nav_lock_until - now));
            }
            true
        } else {
            false
    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        if self.is_loading_archive || self.manager.current != target {
            ...
            if is_ready {
                ...
                // フォルダ開始時のロード完了タイミングなら、ここから2秒のタイマーを開始
                if self.nav_lock_until == f64::MAX {
                    self.nav_lock_until = self.last_display_change_time + NAV_GUARD_DURATION;
                }
        ...
        if (self.error.is_some() || (self.manager.entries.is_empty() && !self.manager.is_listing)) && self.nav_lock_until == f64::MAX {
            self.nav_lock_until = 0.0;
        }
