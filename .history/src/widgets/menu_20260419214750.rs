...
pub fn main_menu_bar(
    ...
    TopBottomPanel::top("top_panel").show(ctx, |ui| {
        menu::bar(ui, |ui| {
            if let Some(act) = ui.menu_button("ファイル", |ui| file_menu(ui, config, manager)).inner.flatten() { action = Some(act); }
            if let Some(act) = ui.menu_button("表示", |ui| view_menu(ui, config, manager, view, show_tree)).inner.flatten() { action = Some(act); }
            if let Some(act) = ui.menu_button("フォルダ", |ui| folder_menu(ui, config)).inner.flatten() { action = Some(act); }
            if let Some(act) = ui.menu_button("オプション", |ui| option_menu(ui, config, show_debug)).inner.flatten() { action = Some(act); }
        });
    });
    action
}
...
pub fn context_menu(
    ...
    // ポップアップ内では縦並びで各メニューを表示。ヘルパーを使い回す。
    if let Some(act) = ui.menu_button("ファイル", |ui| file_menu(ui, config, manager)).inner.flatten() { action = action.or(Some(act)); }
    if let Some(act) = ui.menu_button("表示", |ui| view_menu(ui, config, manager, view, show_tree)).inner.flatten() { action = action.or(Some(act)); }
    if let Some(act) = ui.menu_button("フォルダ", |ui| folder_menu(ui, config)).inner.flatten() { action = action.or(Some(act)); }
    if let Some(act) = ui.menu_button("オプション", |ui| option_menu(ui, config, show_debug)).inner.flatten() { action = action.or(Some(act)); }
