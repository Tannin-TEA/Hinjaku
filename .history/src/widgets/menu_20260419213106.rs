...
pub fn context_menu(
    ...
    // ポップアップ内では縦並びで各メニューボタン（サブメニュー）を表示
    if let Some(act) = ui.menu_button("ファイル", |ui| { /* 既存のファイルメニュー内容 */ }).inner { action = action.or(act); }
    if let Some(act) = ui.menu_button("表示", |ui| { /* 既存の表示メニュー内容 */ }).inner { action = action.or(act); }
    if let Some(act) = ui.menu_button("フォルダ", |ui| { /* 既存のフォルダメニュー内容 */ }).inner { action = action.or(act); }
    if let Some(act) = ui.menu_button("オプション", |ui| { /* 既存のオプションメニュー内容 */ }).inner { action = action.or(act); }
