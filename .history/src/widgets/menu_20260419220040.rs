...
pub fn context_menu(
    ...
    let mut action = None;
    ui.set_min_width(100.0); // コンテキストメニュー自体の幅（最初のリスト）
    
    // ポップアップ内では、各サブメニューを開く際の幅をスリム(140.0)に指定
    if let Some(act) = ui.menu_button("ファイル", |ui| { ui.set_min_width(140.0); file_menu(ui, config, manager) }).inner.flatten() { action = action.or(Some(act)); }
    if let Some(act) = ui.menu_button("表示", |ui| { ui.set_min_width(140.0); view_menu(ui, config, manager, view, show_tree) }).inner.flatten() { action = action.or(Some(act)); }
    if let Some(act) = ui.menu_button("フォルダ", |ui| { ui.set_min_width(140.0); folder_menu(ui, config) }).inner.flatten() { action = action.or(Some(act)); }
    if let Some(act) = ui.menu_button("オプション", |ui| { ui.set_min_width(140.0); option_menu(ui, config, show_debug) }).inner.flatten() { action = action.or(Some(act)); }
