            ...
            if has {
                ...
                ui.with_layout(Layout::right_to_left(Align::Center), |ui| {
                    ...
                    let label = match view.display_mode {
                        DisplayMode::Fit => {
                            if view.zoom == 1.0 { "Fit".to_string() } else { format!("Fit {}", zoom_pct) }
                        }
                        DisplayMode::WindowFit => {
                            if view.zoom == 1.0 { "WinFit".to_string() } else { format!("WinFit {}", zoom_pct) }
                        }
                        DisplayMode::Manual => {
                            if (view.zoom - 1.0).abs() < 0.001 { "等倍".to_string() } else { zoom_pct }
                        }
                    };
                    ui.label(label);
                });
