    ...
    fn go_next(&mut self, ctx: &egui::Context) {
        ...
        let step = if self.manga_mode {
            ...
            if !has_next {
                ...
                1
            } else if (self.manga_shift && self.current % 2 == 0) || (!self.manga_shift && self.current % 2 != 0) {
                // ペアの開始位置にいる場合のみ2ページ進む
                2
            } else {
                // 同期がズレている（ペアの途中にいる）場合は1ページ進めて同期を戻す
                1
            }
        } else {
            1
...
impl eframe::App for App {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        egui::TopBottomPanel::top("toolbar").show(ctx, |ui| {
            ui.horizontal(|ui| {
                ...
                if has {
                    ...
                    // 見開き表示中か判定してラベルを更新
                    let can_pair = self.current > 0 && !self.is_spread(self.current) && self.current + 1 < self.entries.len() && !self.is_spread(self.current + 1);
                    let is_pairing = self.manga_mode && can_pair;
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            let can_pair = self.current > 0 && tex1_size.x <= tex1_size.y;
