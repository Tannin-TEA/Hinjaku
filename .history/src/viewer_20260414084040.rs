...
impl App {
    ...
    fn ui_dir_tree(&self, ui: &mut egui::Ui, path: PathBuf, ctx: &egui::Context, open_req: &mut Option<PathBuf>) {
        ...
        let header = egui::CollapsingHeader::new(RichText::new(format!("{}{}", icon, filename))
            .color(if is_current { Color32::YELLOW } else { Color32::WHITE }));

        let response = header.show(ui, |ui| {
            // アーカイブファイルそのものを read_dir しようとして失敗するのを防ぐ
            if !is_archive && path.is_dir() {
    ...
    fn go_prev_dir(&mut self, ctx: &egui::Context) {
        ...
        if let Some(pos) = siblings.iter().position(|p| p == curr) {
            if pos > 0 {
                let dest = siblings[pos - 1].clone();
                let src = curr.clone();
                self.move_to_dir(dest, Some(src), self.navigator.open_from_end, ctx);
            }
        }
    }
    fn go_next_dir(&mut self, ctx: &egui::Context) {
        ...
        if let Some(pos) = siblings.iter().position(|p| p == curr) {
            if pos + 1 < siblings.len() {
                let dest = siblings[pos + 1].clone();
                let src = curr.clone();
                self.move_to_dir(dest, Some(src), false, ctx);
            }
        }
    }
        ...
        if self.show_tree {
            egui::SidePanel::left("tree_panel").default_width(200.0).show(ctx, |ui| {
                ...
                egui::ScrollArea::both().show(ui, |ui| {
                    let start_path = self.tree_root.clone()
                        .or_else(|| self.navigator.archive_path.as_ref().and_then(|p| p.parent().map(|p| p.to_path_buf())))
                        .or_else(|| std::env::current_dir().ok());

                    if let Some(path) = start_path {
                        self.ui_dir_tree(ui, path, ctx, &mut tree_open_req);
                    }
                });
            });
        }
