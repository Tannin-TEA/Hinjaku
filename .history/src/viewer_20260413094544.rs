...
impl App {
        ...
        match archive::list_images(&path) {
            ...
            Ok(entries) => {
                self.current = if go_last && !entries.is_empty() {
                    ...
                    if self.manga_mode && last > 0 && last % 2 == 0 { last - 1 } else { last }
                } else { 0 };
                self.entries = entries;
    ...
    fn go_prev(&mut self, ctx: &egui::Context) {
        ...
        let step = if self.manga_mode {
            ...
            if self.current <= first_pair_idx {
                1
            } else {
                // 1つ前と2つ前が共に縦長で、かつ現在のインデックスがペアの開始位置なら2ページ戻る
                let is_pair_start = if self.manga_shift {
                    self.current % 2 == 0
                } else {
                    self.current % 2 == 1
                };
                if is_pair_start && !self.is_spread(self.current - 1) && !self.is_spread(self.current - 2) {
                    2
                } else {
                    1
                }
    ...
    fn go_next(&mut self, ctx: &egui::Context) {
        if self.entries.is_empty() { return; }
        let step = if self.manga_mode {
            let is_single_cover = !self.manga_shift && self.current == 0;
            let has_next = self.current + 1 < self.entries.len();
            
            if is_single_cover || self.is_spread(self.current) || !has_next || self.is_spread(self.current + 1) {
                1
            } else if self.manga_shift && self.current % 2 != 0 {
                // シフト中かつ現在がペアの右側（奇数）にいる場合は、位置調整のため1歩
                1
            } else {
                2
            }
...
impl eframe::App for App {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        // ── キーボード ──────────────────────────────────────────────────
        let (left, right, fit_t, zin, zout, manga_t, rcw, rccw, pgup, pgdn, up, dn, p_key, n_key, y_key, s_key) = ctx.input(|i| (
        ...
        // 方向キーの挙動をマンガモードの設定に合わせる
        let (mut go_p, mut go_n) = (p_key, n_key);
        if self.manga_mode && self.manga_rtl {
            if left { go_n = true; }
            if right { go_p = true; }
        } else {
            if left { go_p = true; }
        ...
        if go_p { self.go_prev(ctx); }
        if go_n { self.go_next(ctx); }
        ...
        if s_key && self.manga_mode {
            self.manga_shift = !self.manga_shift;
            self.schedule_prefetch();
            ctx.request_repaint();
        }
        if y_key { self.manga_rtl = !self.manga_rtl; }
        ...
        egui::TopBottomPanel::top("menu").show(ctx, |ui| {
            egui::menu::bar(ui, |ui| {
                ...
                ui.menu_button("表示", |ui| {
                    ...
                    if ui.selectable_label(self.manga_rtl, "ページ方向: 右から左 (Y)").clicked() {
                        self.manga_rtl = !self.manga_rtl;
                        ui.close_menu();
                    }
                    if ui.selectable_label(self.manga_shift, "見開きのズレを修正 (S)").clicked() {
                        self.manga_shift = !self.manga_shift;
                        ui.close_menu();
                    }
                    ui.separator();
        ...
        egui::TopBottomPanel::top("toolbar").show(ctx, |ui| {
            ui.horizontal(|ui| {
                ...
                if self.manga_mode && ui.button("⇆").on_hover_text("見開きのズレを修正 (S)").clicked() {
                    self.manga_shift = !self.manga_shift;
                    self.schedule_prefetch(); ctx.request_repaint();
                }
                ui.separator();
                if has {
                    ...
                    // 見開き表示中か判定してラベルを更新
                    let is_start_index = if self.manga_shift { self.current % 2 == 0 } else { self.current > 0 && self.current % 2 == 1 };
                    let is_pairing = self.manga_mode && is_start_index && !self.is_spread(self.current) 
                                     && self.current + 1 < self.entries.len() && !self.is_spread(self.current + 1);

                    let count = if is_pairing {
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            // 2枚目の取得判定: マンガモード かつ (表紙以外 or シフト中) かつ 1枚目が見開きでない
            let can_pair = if self.manga_shift {
                self.current % 2 == 0
            } else {
                self.current > 0 && self.current % 2 == 1
            };

            let tex2 = if self.manga_mode && can_pair && tex1_size.x <= tex1_size.y {
                    ...
                    if let Some((tex2_id, tex2_size)) = tex2 {
                        ...
                        if resp.clicked() {
                            if let Some(pos) = resp.interact_pointer_pos() {
                                ...
                                if self.manga_rtl {
                                    if is_left { self.go_next(ctx); } else { self.go_prev(ctx); }
                                } else {
