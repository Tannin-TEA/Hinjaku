...
impl App {
    ...
    fn open_external(&mut self, index: usize, ctx: &egui::Context) {
        let Some(curr_path) = self.manager.get_current_full_path() else { return };
        let path_p = utils::to_clean_string(std::path::Path::new(&curr_path));

        // %A: アーカイブファイルまたは親フォルダのパスを確定させる
        let path_a = if let Some(p) = &self.manager.archive_path {
