...
impl App {
    ...
    fn is_nav_locked(&self, ctx: &egui::Context) -> bool {
        ...
        // 表示画像（current）がリクエスト（target）に追いつくまで、次の入力を一切受け付けない
    ...
    fn sync_display_to_target(&mut self, ctx: &egui::Context) {
        ...
        if self.is_loading_archive || self.manager.current != target {
            ...
            // 画像が準備できたか、あるいはエラーが発生したなら表示を切り替える（＝ロックを解除する）
            if self.get_texture(target).is_some() || self.error.is_some() {
                is_ready = true;
            }
            if is_ready {
