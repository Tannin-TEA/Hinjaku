...
struct LoadResult {
    index: usize,
    key: String,
    data: Result<Vec<FrameData>, String>, // 単発画像でも要素1のVecとして扱う
}
...
impl ImageLoader {
            ...
            std::thread::spawn(move || {
                ...
                while let Ok(req) = rx.lock().unwrap().recv() {
                    ...
                    let result_data = (|| -> Result<Vec<FrameData>, String> {
                        ...
                        // アニメーション対応 (30MB以下のGIFのみ)
                        // ※WebPのアニメーションデコードはimage crateのバージョンにより実装が異なるため一旦GIFを優先
                        if is_gif && bytes.len() <= size_limit {
                            ...
                            for frame in frames {
                                ...
                                output.push(FrameData { image: img, delay_ms: delay as u32 });
                            }
    ...
    pub fn collect_results(&mut self, ctx: &egui::Context, linear_filter: bool, entries: &[String]) -> Vec<(usize, String)> {
        ...
        while let Ok(result) = self.load_rx.try_recv() {
            ...
            // インデックスと名前の不一致をチェック（フォルダ切り替え時の古いスレッド結果を捨てる）
            if let Some((_, entry_name)) = result.key.split_once(':') {
                if entries.get(result.index).map(|s| s.as_str()) != Some(entry_name) {
                    continue;
                }
            }
