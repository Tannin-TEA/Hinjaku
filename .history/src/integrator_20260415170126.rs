...
pub fn launch_external(exe: &str, args_tmpl: &[String], path_p: &str, path_a: &str) {
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        unsafe {
            ShellExecuteW(
                std::ptr::null_mut(), // HWND (null_mut)
                operation.as_ptr(),
                exe_u16.as_ptr(),
                parameters.as_ptr(),
                std::ptr::null(),
...
pub fn reveal_in_explorer(path: &Path) {
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        unsafe {
            // パスから PIDL (Windows 内部の識別子) を作成
            let pidl = ILCreateFromPathW(path_u16.as_ptr());
            if !pidl.is_null() {
                // cidl=0, apidl=null で呼び出すと、pidlfolder 自体が選択対象として開かれます
                let _ = SHOpenFolderAndSelectItems(pidl, 0, std::ptr::null(), 0);
                ILFree(pidl); // 使用した PIDL を解放
            }
        }
    }
    #[cfg(not(target_os = "windows"))]
