...
impl Manager {
    ...
    fn process_load_request(
            ...
            match frames_res {
                Ok(frames) => {
                    ...
                    for frame in frames {
                        ...
                        let img = Self::downscale_if_needed(img, req.max_dim, req.filter_mode);
                        let img = Self::apply_rotation(img, req.rotation);
                        let delay_ms = frame.delay().numer_denom_ms().map(|(n, d)| {
                            if d > 0 { n / d } else { loading::DEFAULT_ANIM_FRAME_DELAY_MS }
                        }).unwrap_or(loading::DEFAULT_ANIM_FRAME_DELAY_MS);
        ...
        let img = Self::downscale_if_needed(img.to_rgba8(), req.max_dim, req.filter_mode);
        let img = Self::apply_rotation(img, req.rotation);
    ...
    pub fn schedule_prefetch(&mut self, filter_mode: FilterMode, manga: bool) {
        ...
        let lo = self.target_index.saturating_sub(cache::PREFETCH_BEHIND);
        let hi = (self.target_index + cache::PREFETCH_AHEAD + 1).min(len);
