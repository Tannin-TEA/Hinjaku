        ...
        .show(ctx, |ui| {
            ...
            egui::ScrollArea::vertical().max_height(scroll_height).show(ui, |ui| {
                ...
                egui::Grid::new("mouse_buttons_grid").num_columns(2).spacing([12.0, 6.0]).show(ui, |ui| {
                    let mouse_actions = [
                        "None", "PrevPage", "NextPage", "PrevPageSingle", "NextPageSingle", "PrevDir", "NextDir", "ToggleFit", "ToggleManga"
                    ];
                    ...
                    ui.label("中ボタン (WheelClick):");
                    egui::ComboBox::from_id_source("mouse_middle_combo")
                        .selected_text(get_action_label(&config.mouse_middle_action))
                        .show_ui(ui, |ui| {
                            for act in mouse_actions {
                                if ui.selectable_label(config.mouse_middle_action == act, get_action_label(act)).clicked() {
                                    config.mouse_middle_action = act.to_string();
                                    changed = true;
                                }
                            }
                        });
