...
impl App {
    ...
    fn go_single_prev(&mut self, ctx: &egui::Context) {
        let now = ctx.input(|i| i.time);
        if self.manager.target_index == 0 && (now - self.last_display_change_time) < 0.05 {
            return;
        }
...
impl eframe::App for App {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        // ── キーボード ──────────────────────────────────────────────────
        let (left, right, up, dn, enter_key, esc_key, t_key, fit_t, zin, zout, manga_t, rcw, rccw, pgup, pgdn, p_key, n_key, s_key, home, end, bs_key, e_key, i_key, alt_pressed, y_key, q_key) = ctx.input(|i| (
        ...
        if self.show_tree && !modal_open {
            ...
            if q_key { ctx.send_viewport_cmd(egui::ViewportCommand::Close); }
