...
impl App {
    pub fn new(cc: &eframe::CreationContext<'_>, initial_path: Option<PathBuf>) -> Self {
        ...
        // config.ini (中身はTOML形式) の読み込み、存在しない場合は作成
    ...
    fn go_prev_dir(&mut self, ctx: &egui::Context) {
        ...
        if ctx.input(|i| i.time) - self.last_display_change_time < 0.5 { return; }
    ...
    fn go_single_prev(&mut self, ctx: &egui::Context) {
        ...
        let delay = if self.target_index == 0 { 0.5 } else { 0.1 };
    ...
    fn open_external(&self) {
        ...
        let target_str = if path.is_dir() {
            path.join(entry).to_string_lossy().to_string()
        } else {
            // アーカイブ内の仮想パス。OS標準のセパレータ（Windowsなら \）で結合
            format!("{}{}{}", path.to_string_lossy().trim_end_matches(|c| c == '\\' || c == '/'), std::path::MAIN_SEPARATOR, entry)
        };

        // 徹底的にクリーンアップ：前後の空白を除去し、末尾にセパレータが残っていれば全て削除
        let target_str = target_str.trim().trim_end_matches(|c| c == '\\' || c == '/').to_string();
...
fn downscale_if_needed(img: image::RgbaImage, max_dim: u32, linear: bool) -> image::RgbaImage {
    ...
    let filter = if linear { image::imageops::FilterType::Triangle } else { image::imageops::FilterType::Nearest };
...
impl eframe::App for App {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        // ── キーボード ──────────────────────────────────────────────────
        let (left, right, fit_t, zin, zout, manga_t, rcw, rccw, pgup, pgdn, up, dn, p_key, n_key, s_key, home, end, bs_key, e_key, i_key) = ctx.input(|i| (
        ...
        if s_key { self.manga_shift = !self.manga_shift; }
        if i_key {
        ...
        egui::TopBottomPanel::top("menu").show(ctx, |ui| {
            egui::menu::bar(ui, |ui| {
                ...
                ui.menu_button("表示", |ui| {
                    ...
                    if ui.selectable_label(self.manga_shift, "ページを1つずらす (S)").clicked() {
        ...
        egui::TopBottomPanel::bottom("toolbar").show(ctx, |ui| {
            ui.horizontal(|ui| {
                ...
                ui.separator();
                if has {
                    let name  = self.entries[self.target_index].clone();
