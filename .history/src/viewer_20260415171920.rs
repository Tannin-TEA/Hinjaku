...
impl App {
    ...
    fn add_toast(&mut self, msg: String, ctx: &egui::Context) {
        let expires = ctx.input(|i| i.time) + TOAST_DURATION;
        self.toasts.push((msg, expires));
        // 通知が消えるタイミングで再描画を予約
        ctx.request_repaint_after(std::time::Duration::from_secs_f64(TOAST_DURATION));
    }
...
impl eframe::App for App {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        if self.show_tree && !modal_open {
            ...
            if k.e { self.open_external(0, ctx); } // Eキーは1番目のアプリを起動
