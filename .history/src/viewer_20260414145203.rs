...
impl App {
    ...
    /// アニメーション対応のテクスチャ取得。
    fn get_texture_animated(&self, index: usize, _ctx: &egui::Context) -> Option<(&TextureHandle, egui::Vec2)> {
    ...
    fn rotate_current(&mut self, _cw: bool, ctx: &egui::Context) {
        ...
        if t_key { 
            ...
            if self.show_tree {
                if let Some(path) = &self.manager.archive_path {
                    ...
                    self.manager.tree.selected = Some(path.clone());
                }
            }
            ctx.request_repaint();
        }
