...
impl eframe::App for App {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        // ── キーボード ──────────────────────────────────────────────────
        let (left, right, fit_t, zin, zout, manga_t, rcw, rccw, pgup, pgdn, up, dn, p_key, n_key, s_key, home, end, bs_key, e_key, i_key, enter_key, alt_pressed) = ctx.input(|i| (
        ...
        if !modal_open && enter_key {
            if alt_pressed {
                // Alt + Enter: ボーダレス最大化（タスクバーを残す枠なし最大化）
                self.is_fullscreen = false;
                self.is_borderless = !self.is_borderless;

                // 一旦通常の全画面を解除
                ctx.send_viewport_cmd(egui::ViewportCommand::Fullscreen(false));
                // ボーダレス状態に合わせて枠と最大化を制御
                ...
                // Enter: 全画面（タスクバーも隠す完全な全画面）
                self.is_borderless = false;
                self.is_fullscreen = !self.is_fullscreen;

                ctx.send_viewport_cmd(egui::ViewportCommand::Fullscreen(self.is_fullscreen));
                // 全画面を解除した時は、確実に枠を表示し最大化も解除する
