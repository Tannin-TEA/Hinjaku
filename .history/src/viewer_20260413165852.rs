...
impl App {
            ...
            Ok(entries) => {
                if let Some(ref name) = start_name {
                    self.current = entries.iter().position(|e| {
                        std::path::Path::new(&e.name).file_name()
    ...
    fn go_single_prev(&mut self, ctx: &egui::Context) {
        ...
        let delay = if self.target_index == 0 { 0.1 } else { 0.05 };
...
impl eframe::App for App {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        if s_key { self.show_sort_settings = !self.show_sort_settings; }
        if i_key {
        ...
        if self.show_sort_settings {
            egui::Window::new("並べ替えの設定 (S)")
                ...
                .show(ctx, |ui| {
                    ...
                    if ui.checkbox(&mut self.config.sort_natural, "自然順（数字の大きさを考慮）").on_hover_text("1, 2, 10 の順に並べます。オフにすると 1, 10, 2 の順になります。").changed() {
                        changed = true;
                    }
