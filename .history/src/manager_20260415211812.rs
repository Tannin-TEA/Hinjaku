...
pub struct Manager {
    ...
    pub is_listing: bool,
    pub current_exif: Option<String>,
...
impl Manager {
            ...
            std::thread::spawn(move || {
                ...
                while let Ok(req) = rx.lock().map_err(|_| "Poisoned").and_then(|l| l.recv().map_err(|_| "RecvError")) {
                    ...
                    let result_data = (|| -> std::result::Result<Vec<FrameData>, String> {
                        ...
                        // Exif情報の解析 (軽量な文字列として抽出)
                        let mut exif_info = None;
                        if ext.ends_with(".jpg") || ext.ends_with(".jpeg") || ext.ends_with(".tif") || ext.ends_with(".tiff") {
                            if let Ok(exif_data) = exif::Reader::new().read_from_container(&mut std::io::Cursor::new(&bytes)) {
                                let mut parts = Vec::new();
                                let tags = [
                                    (exif::Tag::Model, "カメラ"),
                                    (exif::Tag::LensModel, "レンズ"),
                                    (exif::Tag::ExposureTime, "露出"),
                                    (exif::Tag::FNumber, "F値"),
                                    (exif::Tag::PhotographicSensitivity, "ISO"),
                                    (exif::Tag::DateTimeOriginal, "撮影日時"),
                                ];
                                for (tag, label) in tags {
                                    if let Some(f) = exif_data.get_field(tag, exif::In::PRIMARY) {
                                        parts.push(format!("{}: {}", label, f.display_value()));
                                    }
                                }
                                // GPS情報の取得
                                if let Some(field) = exif_data.get_field(exif::Tag::GPSLatitude, exif::In::PRIMARY) {
                                    ...
                                    parts.push(format!("緯度: {} {}", lat_ref, field.display_value()));
                                }
                                if let Some(field) = exif_data.get_field(exif::Tag::GPSLongitude, exif::In::PRIMARY) {
                                    let lon_ref = exif_data.get_field(exif::Tag::GPSLongitudeRef, exif::In::PRIMARY)
                                        .map(|f| f.display_value().to_string()).unwrap_or_default();
                                    parts.push(format!("経度: {} {}", lon_ref, field.display_value()));
                                }
                                if !parts.is_empty() { exif_info = Some(parts.join("\n")); }
                            }
                        }
                    ...
                    let _ = tx.send(LoadResult { index: req.index, key: req.key, data: result_data, exif: exif_info, generation: req.generation });
    ...
    pub fn update(&mut self, ctx: &egui::Context, config: &Config) -> Vec<(usize, String)> {
        ...
        while let Ok(result) = self.load_rx.try_recv() {
            ...
            // 表示対象のインデックスのExifが届いたら保持する
            if result.index == self.target_index {
                self.current_exif = result.exif.clone();
            }
