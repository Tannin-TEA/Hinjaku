...
impl eframe::App for App {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        let modal_open = self.show_sort_settings || self.show_settings || self.show_key_config;
        ...
        if self.show_tree && !modal_open {
            ...
            if k.open_external { self.open_external(0, ctx); }
        ...
        if self.show_key_config {
            if widgets::key_config_window(ctx, &mut self.show_key_config, &mut self.config) {
                self.save_config();
            }
        }
