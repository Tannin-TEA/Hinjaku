...
impl App {
        ...
        std::thread::spawn(move || {
            while let Ok(req) = req_rx.recv() {
                let result = (|| -> Option<LoadResult> {
                    ...
                    if dist > 10 { return None; } // 10ページ以上離れたら古いとみなす
    ...
    fn schedule_prefetch(&mut self) {
        ...
        // 共有インデックスを更新
        self.current_idx_shared.store(self.current, Ordering::Relaxed);
        ...
        self.request_load(self.current);
        if self.manga_mode && self.current + 1 < len {
            self.request_load(self.current + 1);
        }
    ...
    fn go_prev(&mut self, ctx: &egui::Context) {
        ...
        let step = if self.manga_mode {
            ...
            if self.current <= first_pair_idx || self.current < 2 {
                1
            } else {
                let prev_is_spread = self.is_spread(self.current - 1);
                let prev_prev_is_spread = self.is_spread(self.current - 2);
    ...
    fn go_next(&mut self, ctx: &egui::Context) {
        ...
        let step = if self.manga_mode {
            if self.current + 1 >= self.entries.len() {
                1
            } else if !self.manga_shift && self.current == 0 {
                1
            } else if self.is_spread(self.current) || self.is_spread(self.current + 1) {
                1
            } else {
                2
            }
        ...
        egui::TopBottomPanel::top("toolbar").show(ctx, |ui| {
            ui.horizontal(|ui| {
                ...
                if has {
                    ...
                    // 見開き表示中か判定してラベルを更新
                    let pairing_rendered = (self.manga_shift || self.current > 0) && !self.is_spread(self.current) 
                        && self.current + 1 < self.entries.len() && !self.is_spread(self.current + 1);

                    let count = if self.manga_mode && pairing_rendered {
                    ...
                    if let Some((tex2_id, tex2_size)) = tex2 {
                        ...
                        if resp.clicked() {
                            if let Some(pos) = resp.interact_pointer_pos() {
                                ...
                                if self.manga_rtl { is_left = !is_left; }
