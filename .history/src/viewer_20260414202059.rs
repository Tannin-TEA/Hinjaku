    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        if self.is_loading_archive || self.manager.current != target {
            ...
            if is_ready {
                ...
                // フォルダ開始時のロード完了なら長いウェイト、それ以外（通常のページ捲り完了）は短いウェイトをかける
                if was_loading { self.folder_lock_until = now + FOLDER_NAV_GUARD_DURATION; }
