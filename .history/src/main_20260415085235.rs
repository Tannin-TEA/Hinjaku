...
fn main() -> eframe::Result<()> {
    ...
    let initial_path = path_arg.and_then(|p| {
        let p = std::fs::canonicalize(&p).ok().or(Some(p))?;
        let s = p.to_string_lossy();
        // Windowsの \\?\ プレフィックスを除去して比較を正常化する
        if s.starts_with(r"\\?\") { Some(std::path::PathBuf::from(&s[4..])) }
        else { Some(p) }
    });
    ...
    if !config.allow_multiple_instances {
        #[cfg(target_os = "windows")]
        {
            ...
            unsafe {
                ...
                _mutex_handle = handle as isize; // ハンドルをドロップさせないように保持
            }
        }
    }
