use crate::{archive, integrator, config::{self, Config, SortMode, SortOrder}, manager::{self, Manager}, utils, painter, widgets, input};
use eframe::egui::{self, FontData, FontDefinitions, FontFamily, TextureHandle, RichText};
use std::path::PathBuf;

/// 【調整用】アーカイブ・フォルダを新規に開いた直後のロック時間 (秒)
const FOLDER_NAV_GUARD_DURATION: f64 = 0.5;
/// 【調整用】ページめくりやマンガモード同期が完了した直後のロック時間 (秒)
const PAGE_NAV_GUARD_DURATION: f64 = 0.05;
...
impl App {
    ...
    fn get_texture_animated(&self, index: usize, ctx: &egui::Context) -> Option<(&TextureHandle, egui::Vec2)> {
        ...
        let (tex, next_frame_sec) = match self.manager.get_tex(index, now) {
            Some(t) => t,
            None => return None,
        };
    ...
    fn open_external(&self) {
        ...
        integrator::launch_external(&self.config.external_app, &self.config.external_args, &target_str);
    }
...
impl eframe::App for App {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        if self.show_tree && !modal_open {
            ...
            // 通常ビューアモード
            if k.left || k.p { self.go_prev(ctx); }
            if k.right || k.n { self.go_next(ctx); }

            if k.up { if self.manga_mode { self.go_single_prev(ctx); } else { self.go_prev(ctx); } }
            if k.dn { if self.manga_mode { self.go_single_next(ctx); } else { self.go_next(ctx); } }

            if k.home { self.go_first(ctx); }
            if k.end  { self.go_last(ctx); }
            if k.fit_t {
            ...
            if k.zin    { self.zoom = (self.zoom * ZOOM_STEP).min(MAX_ZOOM); self.display_mode = DisplayMode::Manual; }
            if k.zout   { self.zoom = (self.zoom / ZOOM_STEP).max(MIN_ZOOM); self.display_mode = DisplayMode::Manual; }
            if k.rcw    { self.rotate_current(true,  ctx); }
            if k.rccw   { self.rotate_current(false, ctx); }
            if k.pgup   { self.go_prev_dir(ctx); }
            if k.pgdn   { self.go_next_dir(ctx); }
            if k.manga_t {
            ...
            if k.b {
                self.config.bg_mode = match self.config.bg_mode {
                    ...
                    config::BackgroundMode::White => config::BackgroundMode::Theme,
                };
                self.save_config();
            }
            if k.bs {
                if let Some(path) = &self.manager.archive_path {
                    integrator::reveal_in_explorer(path);
                }
            }
            if k.e { self.open_external(); }

            if k.q_key || k.ctrl_w { ctx.send_viewport_cmd(egui::ViewportCommand::Close); }
        ...
        if self.show_settings {
            widgets::settings_window(ctx, &mut self.show_settings, &mut self.config, &mut self.settings_args_tmp, || self.save_config());
        }

        // ── ソート設定ウィンドウ ──────────────────────────────────────────
        if self.show_sort_settings {
            widgets::sort_settings_window(ctx, &mut self.show_sort_settings, &mut self.config, &mut self.sort_focus_idx, k.enter, || {
                ...
                .show(ctx, |ui| {
                    ui.vertical(|ui| {
                        ...
                        egui::ScrollArea::both()
                            ...
                            .show(ui, |ui| {
                                let roots = self.manager.archive_reader.get_roots();
                                for root in roots {
                                    widgets::ui_dir_tree(&mut self.manager.tree, &self.manager.archive_path, ui, root, ctx, &mut tree_open_req);
                                }
                            });
            ...
            egui::menu::bar(ui, |ui| {
                ui.menu_button("ファイル", |ui| {
                    ...
                    if ui.add_enabled(self.manager.archive_path.is_some(), egui::Button::new("エクスプローラーで表示 (BS)")).clicked() {
                        ui.close_menu();
                        if let Some(path) = &self.manager.archive_path { integrator::reveal_in_explorer(path); }
                    }
                ...
                ui.menu_button("表示", |ui| {
                    if ui.selectable_label(self.display_mode == DisplayMode::Fit, "フィット表示 (F)").clicked() {
                        self.display_mode = DisplayMode::Fit; ui.close_menu();
                    }
                    if ui.selectable_label(self.display_mode == DisplayMode::WindowFit, "ウィンドウサイズに合わせる").clicked() {
                        self.display_mode = DisplayMode::WindowFit; ui.close_menu();
                    }
                    if ui.selectable_label(self.display_mode == DisplayMode::Manual && self.zoom == 1.0, "等倍表示 (100%)").clicked() {
                        self.display_mode = DisplayMode::Manual; self.zoom = 1.0; ui.close_menu();
                    }
                    ui.separator();
                    if ui.button("拡大 (+)").clicked() { self.zoom = (self.zoom * ZOOM_STEP).min(MAX_ZOOM); self.display_mode = DisplayMode::Manual; ui.close_menu(); }
                    if ui.button("縮小 (-)").clicked() { self.zoom = (self.zoom / ZOOM_STEP).max(MIN_ZOOM); self.display_mode = DisplayMode::Manual; ui.close_menu(); }
                    ui.separator();
                    ...
                    ui.menu_button("背景色", |ui| {
                        let modes = [
                            (config::BackgroundMode::Theme, "アプリ既定"),
                            (config::BackgroundMode::Checkerboard, "市松模様"),
                            (config::BackgroundMode::Black, "黒"),
                            (config::BackgroundMode::Gray, "灰色"),
                            (config::BackgroundMode::White, "白"),
                        ];
                        for (m, label) in modes {
                            if ui.selectable_label(self.config.bg_mode == m, label).clicked() {
                                self.config.bg_mode = m; self.save_config(); ui.close_menu();
                            }
                        }
                    });
                });
        ...
        egui::TopBottomPanel::bottom("toolbar").show(ctx, |ui| {
            ui.horizontal(|ui| {
                ...
                if has {
                    ...
                    ui.label(format!("{}/{}", self.manager.target_index + 1, self.manager.entries.len()));
                }
                ...
                if has {
                    ...
                    let short = utils::get_display_name(std::path::Path::new(&meta.name)); // utils::get_display_name を使用
                    ...
                    let tex = self.get_texture(self.manager.target_index);
                    let res_str = if let Some(t) = tex {
                        let s = t.size();
                        format!("{}x{}", s[0], s[1])
                    } else {
                        "---x---".to_string()
                    };
                    let loading = tex.is_none();
                    let status  = if loading { " ⏳" } else { "" };
                ui.label(format!("{}{} | {} | {} | {} | [{} {}]", 
                    short, status, file_size, res_str, day_str, sort_label, sort_icon));
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            let (tex1_ref, tex1_size) = match tex1 {
                Some(t) => t,
                None => return, // 基本的にここには来ないが安全のため
            };
                ...
                if self.manga_mode {
                    if let Some((tex2_id, tex2_size)) = tex2 {
                        ...
                        if self.config.manga_rtl {
                            ...
                            ui.painter().image(tex1_id, r1, uv, egui::Color32::WHITE);
                            ui.painter().image(tex2_id, r2, uv, egui::Color32::WHITE);
                        } else {
                        ...
                        // 2枚目まだロード中
                        let resp = painter::draw_centered(ui, tex1_id, tex1_size, avail, mode, zoom);
                        if click_allowed && resp.secondary_clicked() {
                            self.go_prev(ctx);
                        } else if click_allowed && resp.clicked() {
                            if let Some(pos) = resp.interact_pointer_pos() {
