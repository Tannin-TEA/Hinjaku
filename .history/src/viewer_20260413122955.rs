...
impl App {
    ...
    fn go_single_prev(&mut self, ctx: &egui::Context) {
        ...
        let delay = if self.target_index == 0 { 0.5 } else { 0.05 };
...
impl eframe::App for App {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        // ── キーボード ──────────────────────────────────────────────────
        let (left, right, fit_t, zin, zout, manga_t, rcw, rccw, pgup, pgdn, up, dn, p_key, n_key, s_key, home, end, bs_key, e_key) = ctx.input(|i| (
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            // マウス操作: 右クリックホールド+ホイール(ズーム) / 通常ホイール(ページ送り)
            ...
            if wheel != 0.0 {
                if ctrl || secondary {
                    ...
                    if wheel > 0.0 { self.go_prev(ctx); } else { self.go_next(ctx); }
                }
            }
