use crate::{archive, integrator, config::{self, Config, SortMode, SortOrder}, manager::{self, Manager}, utils, error::HinjakuError};
...
/// 市松模様のタイルサイズ
const CHECKERBOARD_GRID_SIZE: f32 = 16.0;
/// 市松模様の色1 (暗い)
const CHECKERBOARD_COLOR_1: Color32 = Color32::from_gray(25);
/// 市松模様の色2 (明るい)
const CHECKERBOARD_COLOR_2: Color32 = Color32::from_gray(40);
...
impl App {
    ...
    fn ui_dir_tree(
        ...
        let filename = utils::get_display_name(&path); // utils::get_display_name を使用

        let kind = utils::detect_kind(&path); // utils::detect_kind を使用
        let is_archive = matches!(kind, utils::ArchiveKind::Zip | utils::ArchiveKind::SevenZ); // utils::ArchiveKind を使用
...
fn paint_background(ui: &mut egui::Ui, rect: egui::Rect, mode: config::BackgroundMode) {
    match mode {
...
impl eframe::App for App {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        // ── キーボード ──────────────────────────────────────────────────
        let (left, right, up, dn, enter_key, esc_key, t_key, fit_t, zin, zout, manga_t, rcw, rccw, pgup, pgdn, p_key, n_key, s_key, home, end, bs_key, e_key, i_key, alt_pressed, y_key, q_key, ctrl_w, b_key) = ctx.input(|i| (
        ...
        if self.show_tree && !modal_open {
            ...
            if zin    { self.zoom = (self.zoom * ZOOM_STEP).min(MAX_ZOOM); self.display_mode = DisplayMode::Manual; }
            if zout   { self.zoom = (self.zoom / ZOOM_STEP).max(MIN_ZOOM); self.display_mode = DisplayMode::Manual; }
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            paint_background(ui, ui.available_rect_before_wrap(), self.config.bg_mode);
            ...
            egui::ScrollArea::both().show(ui, |ui| {
                if self.manga_mode {
                    if let Some((tex2_id, tex2_size)) = tex2 {
                        ...
                        // 2枚目まだロード中
                        let resp = draw_centered(ui, tex1_id, tex1_size, avail, mode, zoom);
                        if click_allowed && resp.secondary_clicked() {
                            self.go_prev(ctx);
                        } else if click_allowed && resp.clicked() {
                            if let Some(pos) = resp.interact_pointer_pos() {
