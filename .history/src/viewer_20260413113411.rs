        ...
        if e_key {
            if let Some(path) = &self.archive_path {
                let target = if path.is_dir() {
                    path.join(&self.entries[self.current])
                } else {
                    path.clone()
                };
                
                // 特定のソフトを使いたい場合は以下のように記述：
                // let _ = std::process::Command::new(r"C:\Program Files\YourApp\app.exe").arg(&target).spawn();

                // ここではWindowsの「ファイルを開く」既定動作
            ...
            egui::menu::bar(ui, |ui| {
                ui.menu_button("ファイル", |ui| {
                    ...
                    if ui.add_enabled(self.archive_path.is_some(), egui::Button::new("外部アプリで開く (E)")).clicked() {
                        ui.close_menu();
                        if let Some(path) = &self.archive_path {
                            let target = if path.is_dir() { path.join(&self.entries[self.current]) } else { path.clone() };
                            #[cfg(target_os = "windows")]
                            let _ = std::process::Command::new("cmd")
                                .args(&["/C", "start", "", &target.to_string_lossy()])
                                .spawn();
