...
impl App {
    ...
    fn open_external(&mut self, index: usize, ctx: &egui::Context) {
        let Some(curr_path_str) = self.manager.get_current_full_path() else { return };
        let path_p = utils::to_clean_string(std::path::Path::new(&curr_path_str));

        // %A: アーカイブファイルまたは親フォルダのパスを確定させる
        let path_a = if let Some(p) = self.manager.archive_path.as_ref() {
            // 通常の画像ファイルを開いている場合は、その親フォルダを %A とする
            if p.is_file() && matches!(utils::detect_kind(p), utils::ArchiveKind::Plain) {
                utils::to_clean_string(p.parent().unwrap_or(p))
            } else {
