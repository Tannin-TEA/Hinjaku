...
impl eframe::App for App {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        if self.manager.target_index != self.last_target_index {
            ...
            self.manager.current_exif = None; // ページが変わったら一旦クリア
        ...
        if self.show_exif {
            egui::SidePanel::right("exif_panel")
                ...
                .show(ctx, |ui| {
                    ui.heading("📷 Exif情報");
                    ui.separator();
                    widgets::exif_panel_ui(ui, self.manager.current_exif.as_deref());
                });
        }
        ...
        if let Some(act) = menu_action.or(toolbar_action) {
            match act {
                ...
                widgets::ViewerAction::ToggleExif => { self.show_exif = !self.show_exif; },
