...
pub struct Config {
    ...
    /// リミッターモード時のページ送りガード時間 (秒)
    pub limiter_duration: f32,
}
...
pub fn load_config_file(custom_name: Option<&str>) -> (Config, Option<PathBuf>) {
    ...
    if let Ok(ini) = Ini::load_from_file(&path) {
        ...
        if let Some(sec) = ini.section(Some("Global") as Option<&str>) {
            ...
            if let Some(v) = sec.get("LimiterDuration").or(sec.get("ReadingGuardDuration")).or(sec.get("PreviewGuardDuration")) {
                if let Ok(f) = v.parse::<f32>() { cfg.limiter_duration = f; }
            }
...
pub fn save_config_file(cfg: &Config, path: &std::path::Path) -> Result<()> {
    ...
    ini.with_section(Some("Global"))
        ...
        .set("LimiterDuration", cfg.limiter_duration.to_string())
