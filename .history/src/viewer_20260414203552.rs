...
const FOLDER_NAV_GUARD_DURATION: f64 = 2.0;
/// 【調整用】マンガモードで左右の画像が揃った瞬間のロック時間 (秒)
const MANGA_SYNC_GUARD_DURATION: f64 = 0.05;
...
pub struct App {
    ...
    /// マンガモードの左右同期による操作ロック解除時刻
    manga_sync_until: f64,
...
impl App {
    ...
    fn is_nav_locked(&self, ctx: &egui::Context) -> bool {
        ...
        // 2. ページ同期（マンガモード等の揃え）待ち中のロック
        if self.manga_mode && self.manager.current != self.manager.target_index { return true; }
        ...
        // 4. マンガモードの左右揃え直後のガード
        if now < self.manga_sync_until {
            ctx.request_repaint_after(std::time::Duration::from_secs_f64(self.manga_sync_until - now));
            return true;
        }
    ...
    fn go_single_prev(&mut self, ctx: &egui::Context) {
        ...
        self.folder_lock_until = 0.0; self.manga_sync_until = 0.0;
...
fn paint_background(ui: &mut egui::Ui, rect: egui::Rect, mode: config::BackgroundMode) {
    match mode {
        ...
        config::BackgroundMode::Checkerboard => {
            ...
            let mut gx = (rect.min.x / grid_size).floor();
            while gx * grid_size < rect.max.x {
                let mut gy = (rect.min.y / grid_size).floor();
                while gy * grid_size < rect.max.y {
                    if (gx as i32 + gy as i32) % 2 == 0 {
                        let tile = egui::Rect::from_min_size(
                            egui::pos2(gx * grid_size, gy * grid_size),
                            egui::vec2(grid_size, grid_size)
                        ).intersect(rect);
                        if !tile.is_negative() {
                            ui.painter().rect_filled(tile, 0.0, Color32::from_gray(40));
                        }
                    }
                    gy += 1.0;
                }
...
fn draw_centered(
    ...
    paint_background(ui, img_rect, bg_mode);
...
impl eframe::App for App {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        if self.is_loading_archive || self.manager.current != target {
            ...
            if is_ready {
                ...
                if was_loading {
                    ...
                    // マンガモード中のページ揃え完了時のみ、短い同期ウェイトをかける
                    self.manga_sync_until = now + MANGA_SYNC_GUARD_DURATION;
                }
        ...
        if self.show_tree && !modal_open {
            ...
            if zin    { self.zoom = (self.zoom * ZOOM_STEP).min(10.0); self.display_mode = DisplayMode::Manual; }
            if zout   { self.zoom = (self.zoom / ZOOM_STEP).max(0.1);  self.display_mode = DisplayMode::Manual; }
        ...
        egui::TopBottomPanel::top("top_panel").show(ctx, |ui| {
            egui::menu::bar(ui, |ui| {
                ...
                ui.menu_button("表示", |ui| {
                    ...
                    ui.separator();
                    if ui.button("拡大 (+)").clicked() { self.zoom = (self.zoom * ZOOM_STEP).min(10.0); self.display_mode = DisplayMode::Manual; ui.close_menu(); }
                    if ui.button("縮小 (-)").clicked() { self.zoom = (self.zoom / ZOOM_STEP).max(0.1);  self.display_mode = DisplayMode::Manual; ui.close_menu(); }
                    ui.separator();
                ...
                if self.manga_mode {
                    if let Some((tex2_id, tex2_size)) = tex2 {
                        ...
                        if self.config.manga_rtl {
                            ...
                            paint_background(ui, r1, self.config.bg_mode); paint_background(ui, r2, self.config.bg_mode);
                        ...
                        // 2枚目まだロード中
                        let resp = draw_centered(ui, tex1_id, tex1_size, avail, mode, zoom, self.config.bg_mode);
                        if click_allowed && resp.secondary_clicked() {
                            self.go_prev(ctx);
                        } else if click_allowed && resp.clicked() {
                            if let Some(pos) = resp.interact_pointer_pos() {
