            ...
            std::thread::spawn(move || {
                while let Ok(req) = {
                    ...
                    let result = (|| -> Option<LoadResult> {
                        ...
                        // webp専用デコーダーまたはfallback
                        let img = if req.entry_name.to_lowercase().ends_with(".webp") {
                            match webp::Decoder::new(&bytes).decode() {
                                Some(webp_img) => {
                                    // webpをRGBA8に変換
                                    let width = webp_img.width() as u32;
                                    let height = webp_img.height() as u32;
                                    match image::RgbaImage::from_raw(width, height, webp_img.to_rgba().into_owned()) {
                                        Some(rgba) => rgba,
                                        None => {
                                            eprintln!("Failed to create RGBA from webp: {}", req.entry_name);
                                            return None;
                                        }
                                    }
                                }
                                None => {
                                    eprintln!("Failed to decode webp {}", req.entry_name);
                                    return None;
                                }
                            }
                        } else {
