...
impl App {
    ...
    fn go_prev_dir(&mut self, ctx: &egui::Context) {
        if let Some((dest, src)) = self.navigator.find_neighbor(false) { self.move_to_dir(dest, Some(src), self.navigator.open_from_end, ctx); }
    }
    fn go_next_dir(&mut self, ctx: &egui::Context) {
        if let Some((dest, src)) = self.navigator.find_neighbor(true) { self.move_to_dir(dest, Some(src), false, ctx); }
    }
    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        for (idx, err) in failures {
            if idx == self.navigator.target_index {
                self.error = Some(format!("画像の表示に失敗しました: {}", err));
                    ...
                    if self.navigator.entries.is_empty() {
                        ui.vertical_centered(|ui| {
                            ...
                            ui.columns(2, |columns| {
                                columns[0].label("📁 同階層のディレクトリ:");
                                egui::ScrollArea::vertical().id_source("siblings").show(&mut columns[0], |ui| {
                                    if let Some((siblings, current_idx)) = self.navigator.sibling_dirs() {
                                        for (i, path) in siblings.into_iter().enumerate() {
                                            let icon = if path.is_dir() { "📁" } else { "📦" };
                                            let label = format!("{} {}", icon, path.file_name().unwrap_or_default().to_string_lossy());
                                ...
                                columns[1].label("⬇ 下層のディレクトリ:");
                                egui::ScrollArea::vertical().id_source("children").show(&mut columns[1], |ui| {
                                    for path in self.navigator.nav_items.clone() {
                                        let icon = if path.is_dir() { "📁" } else { "📦" };
