...
impl App {
    ...
    fn handle_action(&mut self, ctx: &egui::Context, act: widgets::ViewerAction) {
        match act {
            ...
            widgets::ViewerAction::MoveToCenter => {
                let (wx, wy, ww, wh) = integrator::get_primary_work_area();
                let x = wx + (ww - self.config.window_width) / 2.0;
                let y = wy + (wh - self.config.window_height) / 2.0;
                ctx.send_viewport_cmd(egui::ViewportCommand::OuterPosition(egui::pos2(x, y)));
            }
            widgets::ViewerAction::ToggleWindowCentered => {
                self.config.window_centered = !self.config.window_centered;
                if self.config.window_centered {
                    self.applied_initial_center = false; // フラグを戻して update 内のロジックで移動させる
                }
                self.save_config();
            }
...
impl eframe::App for App {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        // 2. 起動時（または設定変更時）にワークエリア基準で中央配置を実行する
