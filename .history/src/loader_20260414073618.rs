...
impl ImageLoader {
            ...
            std::thread::spawn(move || {
                while let Ok(req) = rx.lock().unwrap().recv() {
                    ...
                    let result_data = (|| -> Result<image::RgbaImage, String> {
                        let bytes = archive::read_entry(&req.archive_path, &req.entry_name)
    ...
    pub fn schedule_prefetch(&mut self, target: usize, archive_path: &Path, entries: &[String], rotations: &HashMap<String, Rotation>, manga_mode: bool, linear_filter: bool) {
        let is_archive = matches!(archive::detect_kind(archive_path), archive::ArchiveKind::Zip | archive::ArchiveKind::SevenZ);
        let len = entries.len();
        if len == 0 { return; }
        ...
        // アーカイブの場合はファイルアクセス競合を防ぐため、先読みループをスキップする
        if !is_archive {
