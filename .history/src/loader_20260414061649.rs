...
fn log_to_file(msg: &str) {
    if let Ok(mut file) = OpenOptions::new().create(true).append(true).open("hinjaku_debug.log") {
...
impl ImageLoader {
    ...
    pub fn schedule_prefetch(&mut self, target: usize, archive_path: &Path, entries: &[String], rotations: &HashMap<String, Rotation>, manga_mode: bool, linear_filter: bool) {
        ...
        self.cache.retain(|k, _| self.is_in_range(k, lo, hi));
        self.cache_lru.retain(|k| self.cache.contains_key(k));
        self.pending.retain(|k| self.is_in_range(k, lo, hi));
    }
    ...
    fn is_in_range(&self, key: &str, lo: usize, hi: usize) -> bool {
