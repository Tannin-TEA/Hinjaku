...
impl App {
    ...
    fn rotate_current(&mut self, cw: bool, ctx: &egui::Context) {
        let Some(path) = self.manager.archive_path.clone() else { return };
        ...
        for idx in indices {
            if let Some(name) = self.manager.entries.get(idx).cloned() {
                let rot = self.manager.rotations.get(&name).copied().unwrap_or(Rotation::R0);
                let new_rot = if cw { rot.cw() } else { rot.ccw() };
                self.manager.rotations.insert(name.clone(), new_rot);
                self.manager.invalidate_cache_for(idx, &path, &name);
            }
        }
