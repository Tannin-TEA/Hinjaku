...
impl App {
    ...
    fn is_nav_locked(&self, ctx: &egui::Context) -> bool {
        ...
        // リミッターモードがONの場合、表示が追いつくまで操作をロックする
        if self.config.limiter_mode && self.manager.current != self.manager.target_index {
            return true;
        }
    ...
    fn handle_viewer_keys(&mut self, ctx: &egui::Context, k: &input::KeyboardState) {
        ...
        if k.toggle_limiter {
            self.handle_action(ctx, ToggleLimiterMode);
        }
        if k.toggle_tree {
    ...
    fn sync_display_to_target(&mut self, ctx: &egui::Context) {
        ...
        if self.is_loading_archive || self.manager.current != target {
            ...
            if is_ready {
                ...
                self.is_loading_archive = false;
                let now = ctx.input(|i| i.time);
                let guard = if self.config.limiter_mode { self.config.limiter_duration as f64 } else { ui::PAGE_NAV_GUARD_DURATION };
    ...
    fn handle_action(&mut self, ctx: &egui::Context, act: widgets::ViewerAction) {
        ...
        match act {
            ...
            SetLimiterDuration(d) => {
                self.config.limiter_duration = d;
                self.save_config();
            }
