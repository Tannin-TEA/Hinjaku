use crate::{archive, integrator, config::{self, Config, SortMode, SortOrder}, manager::{self, Manager}, utils}; // utils モジュールをインポート
...
impl App {
    ...
    fn get_texture_animated(&self, index: usize, ctx: &egui::Context) -> Option<(&TextureHandle, egui::Vec2)> {
        ...
        let (tex, next_frame_sec) = self.manager.get_tex(index, now)?; // manager のメソッド
    ...
    fn save_config(&self) {
        if let Some(ref path) = self.config_path {
            config::save_config_file(&self.config, path);
        }
    }
}
