...
pub struct Manager {
    ...
    archive_reader: Arc<dyn ArchiveReader>, // ArchiveReader トレイトへの参照
}
...
impl Manager {
    ...
    pub fn open_path(&mut self, path: PathBuf, _config: &Config) {
        ...
        let (base_path, start_name) = if path.is_file() && crate::utils::is_image_ext(&path.to_string_lossy()) { // utils へ移動
...
impl NavTree {
    ...
    pub fn get_image_count(&mut self, path: &Path) -> usize {
        ...
        let count = self.archive_reader.list_images(&path).map(|e| e.len()).unwrap_or(0); // ArchiveReader 経由
        self.image_counts.insert(path, count);
        count
    }
    pub fn expand_to_path(&mut self, path: &Path) {
        let mut curr = Some(crate::utils::clean_path(path)); // utils へ移動
