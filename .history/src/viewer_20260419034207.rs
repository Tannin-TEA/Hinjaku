use crate::{archive, integrator, window, shell, config::{self, Config}, manager::{self, Manager}, utils, painter, widgets, input};
...
impl App {
    ...
    fn handle_action(&mut self, ctx: &egui::Context, act: widgets::ViewerAction) {
        match act {
            ...
            widgets::ViewerAction::RevealInExplorer => { if let Err(e) = shell::reveal_current_in_explorer(&self.manager) { self.add_toast(e, ctx); } }
