...
pub fn send_path_to_existing_instance(path: &Path) -> bool {
    #[cfg(target_os = "windows")]
    {
        ...
        use windows_sys::Win32::Foundation::{GENERIC_WRITE, INVALID_HANDLE_VALUE, CloseHandle};

        let pipe_name: Vec<u16> = "\\\\.\\pipe\\HinjakuIPC\0".encode_utf16().collect();
        unsafe {
            // パイプが利用可能になるまで短時間待機
            if WaitNamedPipeW(pipe_name.as_ptr(), 1000) == 0 { return false; }
            ...
            FlushFileBuffers(handle); // 確実にデータを送り出す
            CloseHandle(handle);
            true
        }
    }
...
/// IPCサーバーを開始して他のインスタンスからのパス入力を待機する
pub fn start_ipc_server(tx: Sender<PathBuf>) {
    #[cfg(target_os = "windows")]
    {
        std::thread::spawn(move || {
            ...
            // windows-sys 0.59.0 で解決できない場合があるため、定数を定義
            ...
            let handle = unsafe { CreateNamedPipeW(
                ...
                10, 4096, 4096, 0, std::ptr::null()
            )};
            if handle == INVALID_HANDLE_VALUE { return; }

            loop {
                if unsafe { ConnectNamedPipe(handle, std::ptr::null_mut()) } != 0 || unsafe { GetLastError() } == ERROR_PIPE_CONNECTED {
                    let mut buffer = [0u8; 4096];
                    let mut read = 0;
                    if unsafe { ReadFile(handle, buffer.as_mut_ptr() as _, buffer.len() as u32, &mut read, std::ptr::null_mut()) } != 0 && read > 0 {
