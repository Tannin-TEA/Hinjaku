    ...
    fn handle_action(&mut self, ctx: &egui::Context, act: widgets::ViewerAction) {
        match act {
            ...
            widgets::ViewerAction::MoveToCenter => {
                ...
                let x = screen_rect.min.x + (screen_rect.width() - self.config.window_width) / 2.0;
                let y = screen_rect.min.y + (screen_rect.height() - self.config.window_height) / 2.0;
                ctx.send_viewport_cmd(egui::ViewportCommand::OuterPosition(egui::pos2(x, y)));
            }
            widgets::ViewerAction::ToggleWindowCentered => { self.config.window_centered = !self.config.window_centered; self.save_config(); }
