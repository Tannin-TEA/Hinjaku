...
impl App {
    ...
    fn go_prev(&mut self, ctx: &egui::Context) {
        ...
        let step = if self.manga_mode {
            if self.current <= 1 {
                ...
                // 1つ前と2つ前が共に縦長なら2ページ戻る（ペア表示だったと推測）
                if !self.is_spread(self.current - 1) && !self.is_spread(self.current - 2) && (self.current - 2) > 0 {
                    2
                } else {
                    1
                }
    ...
    fn go_next(&mut self, ctx: &egui::Context) {
        if self.entries.is_empty() { return; }
        let step = if self.manga_mode {
            // 表紙(0)、現在が見開き、または次が見開きなら1ページだけ進む
            if self.current == 0 || self.is_spread(self.current) || self.current + 1 >= self.entries.len() || self.is_spread(self.current + 1) {
                1
            } else {
                2
            }
...
impl eframe::App for App {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        // ── キーボード ──────────────────────────────────────────────────
        let (left, right, fit_t, zin, zout, manga_t, rcw, rccw, pgup, pgdn, up, dn, p_key, n_key, y_key) = ctx.input(|i| (
        ...
        if manga_t {
            ...
            // 表紙(0)以外で偶数ページにいたら、ペアの開始位置(奇数)に調整
            if self.manga_mode && self.current > 0 && self.current % 2 == 0 {
                self.current -= 1;
            }
            self.schedule_prefetch();
            ctx.request_repaint();
        ...
        egui::TopBottomPanel::top("toolbar").show(ctx, |ui| {
            ui.horizontal(|ui| {
                ...
                if ui.button(ml).on_hover_text("マンガモード (M)").clicked() {
                    self.manga_mode = !self.manga_mode;
                    if self.manga_mode && self.current > 0 && self.current % 2 == 0 { self.current -= 1; }
                ...
                if has {
                    ...
                    // 見開き表示中か判定してラベルを更新
                    let is_pairing = self.manga_mode && self.current > 0 && !self.is_spread(self.current) 
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            // 2枚目の取得判定: マンガモード かつ 表紙以外 かつ 1枚目が見開きでない
            let tex2 = if self.manga_mode && self.current > 0 && tex1_size.x <= tex1_size.y {
