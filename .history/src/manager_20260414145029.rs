...
impl Manager {
            ...
            std::thread::spawn(move || {
                ...
                while let Ok(req) = rx.lock().unwrap().recv() {
                    ...
                    let result_data = (|| -> Result<Vec<FrameData>, String> {
                        ...
                        if (ext.ends_with(".gif") || ext.ends_with(".webp")) && bytes.len() <= 30*1024*1024 {
                            // アニメーションデコードロジック（簡易化のため今回は静止画優先、必要なら前回のコードをここに統合）
                        }
        ...
        while let Ok(result) = self.load_rx.try_recv() {
            ...
            match result.data {
                Ok(frames) => {
                    ...
                    self.cache.insert(result.key.clone(), CachedImage::Static(tex));
    ...
    pub fn get_tex(&self, index: usize) -> Option<&TextureHandle> {
        let path = self.archive_path.as_ref()?;
        let entry = self.entries.get(index)?;
        let key = format!("{}:{}:{}", index, path.to_string_lossy(), entry);
        self.cache.get(&key).map(|c| match c { CachedImage::Static(t) => t, _ => unreachable!() })
    }

    pub fn is_spread(&self, index: usize) -> bool {
        self.get_tex(index).map(|t| t.size_vec2().x > t.size_vec2().y).unwrap_or(false)
    }
    ...
    pub fn get_current_full_path(&self) -> Option<String> {
        ...
        let combined = if path.is_dir() { path.join(entry).to_string_lossy().to_string() }
        else { format!("{}\\{}", path.to_string_lossy().trim_end_matches(|c| c=='\\'||c=='/'), entry.trim_start_matches(|c| c=='\\'||c=='/')) };
...
impl NavTree {
    ...
    // --- 以下のメソッドは Viewer からの移行用 ---

    pub fn move_selection(&mut self, delta: isize) {
        // 簡易実装: 現在の選択から兄弟要素内を移動
    ...
    pub fn expand_current(&mut self) {
        if let Some(sel) = self.selected.clone() {
            if archive::detect_kind(&sel) == archive::ArchiveKind::Plain {
