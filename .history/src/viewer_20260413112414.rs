...
impl eframe::App for App {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        // ── キーボード ──────────────────────────────────────────────────
        let (left, right, fit_t, zin, zout, manga_t, rcw, rccw, pgup, pgdn, up, dn, p_key, n_key, y_key, s_key, home, end) = ctx.input(|i| (
        ...
        egui::TopBottomPanel::top("toolbar").show(ctx, |ui| {
            ui.horizontal(|ui| {
                ...
                if has {
                    ...
                    // 見開き表示中か判定してラベルを更新
                    let pairing_rendered = (self.manga_shift || self.target_index > 0) && !self.is_spread(self.target_index) 
                        && self.target_index + 1 < self.entries.len() && !self.is_spread(self.target_index + 1);

                    let count = if self.manga_mode && pairing_rendered {
                        format!("{}-{} / {}", self.target_index+1, self.target_index+2, self.entries.len())
                    } else {
                        format!("{} / {}", self.target_index+1, self.entries.len())
                    };
                    // ファイルサイズ取得
