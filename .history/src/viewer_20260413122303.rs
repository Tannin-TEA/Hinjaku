...
impl App {
    ...
    fn open_external(&self) {
        ...
        let mut target_str = if path.is_dir() {
            path.join(entry).to_string_lossy().to_string()
        } else {
            // アーカイブ内のパスを "アーカイブパス/内部パス" の形式で結合
            format!("{}/{}", path.to_string_lossy(), entry)
        };

        // ファイルパス（拡張子あり）の場合、末尾にセパレータが付かないよう徹底的に除去
        if target_str.contains('.') {
            target_str = target_str.trim_end_matches(|c: char| c == '\\' || c == '/').to_string();
        }
