...
impl DefaultArchiveReader {
    // ライブラリのバインドを静的に保持して、2回目以降のロードを高速化する
    fn init_pdfium(&self) -> Result<Pdfium> {
        static BINDINGS: std::sync::OnceLock<Box<dyn PdfiumLibraryBindings + Send + Sync>> = std::sync::OnceLock::new();

        let bindings = BINDINGS.get_or_init(|| {
            let exe_dir = std::env::current_exe().ok()
                .and_then(|p| p.parent().map(|p| p.to_path_buf()))
                .unwrap_or_else(|| PathBuf::from("./"));

            let b = Pdfium::bind_to_library(Pdfium::pdfium_platform_library_name_at_path(exe_dir.to_str().unwrap_or("./")))
...
// ── PDF ─────────────────────────────────────────────────────────────────────
    ...
    fn read_pdf(&self, path: &Path, page_index: Option<usize>) -> Result<Vec<u8>> {
        ...
        // 元の縦横比を維持しつつ、長辺が 2000px になるように計算
        let width = page.width().value;
        let height = page.height().value;
        let scale = 2000.0 / width.max(height);
        ...
        let b_width = bitmap.width() as u32;
        let b_height = bitmap.height() as u32;
        
        // pdfium-render のビットマップ(BGRA)を RGBA に変換 (RとBを入れ替える)
        let mut bgra_data: Vec<u8> = bitmap.as_raw_bytes().to_vec();
        for chunk in bgra_data.chunks_exact_mut(4) {
            chunk.swap(0, 2);
        }

        let img = ::image::RgbaImage::from_raw(b_width, b_height, bgra_data)
            .ok_or_else(|| HinjakuError::Archive("Failed to create image buffer from PDF".to_string()))?;

        ::image::DynamicImage::ImageRgba8(img)
            .write_to(&mut std::io::Cursor::new(&mut png_data), ::image::ImageFormat::Png)
            .map_err(|e| HinjakuError::Archive(format!("PNG conversion error: {e}")))?;
