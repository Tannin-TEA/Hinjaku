...
impl App {
    ...
    fn find_adjacent_path_recursive(&self, path: &std::path::Path, forward: bool) -> Option<PathBuf> {
        // パスを正規化して絶対パスを取得
        let abs_path = path.canonicalize().ok().or_else(|| std::path::absolute(path).ok())?;
        let parent = abs_path.parent()?;
        let current_name = abs_path.file_name()?.to_ascii_lowercase();
        ...
        // 現在の位置を特定（大文字小文字を区別しない）
        let idx = siblings.iter().position(|p| {
            p.file_name().map(|f| f.to_ascii_lowercase()) == Some(current_name.clone())
        })?;
