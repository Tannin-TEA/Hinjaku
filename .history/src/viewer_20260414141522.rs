...
use config::{SortMode, SortOrder, Config};
...
impl App {
    pub fn new(cc: &eframe::CreationContext<'_>, initial_path: Option<PathBuf>, listener: Option<std::net::TcpListener>) -> Self {
        ...
        let mut app = Self {
            ...
            nav_tree: navigator::NavTree::new(),
    ...
    fn schedule_prefetch(&mut self) {
        if let Some(ref path) = self.navigator.archive_path {
            self.loader.schedule_prefetch(self.navigator.target_index, path, &self.navigator.entries_meta, &self.navigator.rotations, self.manga_mode, self.config.linear_filter);
        }
    }

    fn get_texture(&self, index: usize) -> Option<&TextureHandle> {
        let path = self.navigator.archive_path.as_ref()?;
        let entry = self.navigator.entries.get(index)?;
        let key = format!("{}:{}:{}", index, path.to_string_lossy(), entry);
        // 見開き判定など、フレーム選択不要な箇所用（先頭フレームを返す）
        self.loader.get_first_frame(&key)
    }

    /// アニメーション対応のテクスチャ取得。
    /// 現在時刻に対応するフレームを返し、次フレームまでの時間で repaint を予約する。
    fn get_texture_animated(&self, index: usize, ctx: &egui::Context) -> Option<(&TextureHandle, egui::Vec2)> {
        let path = self.navigator.archive_path.as_ref()?;
        let entry = self.navigator.entries.get(index)?;
        let key = format!("{}:{}:{}", index, path.to_string_lossy(), entry);
        let now = ctx.input(|i| i.time);
        let (tex, next_frame_sec) = self.loader.get_image(&key, now)?;
        // 次フレームのタイミングで再描画を予約
    ...
    fn open_path(&mut self, path: PathBuf, ctx: &egui::Context) {
        ...
        if self.tree_root.is_none() {
            self.tree_root = self.navigator.archive_path.as_ref()
                .and_then(|p| p.parent().map(|parent| parent.to_path_buf()));
        }
        self.is_loading_archive = !self.navigator.entries.is_empty();
    ...
    fn ui_dir_tree(
        nav_tree: &mut navigator::NavTree,
        ...
        let is_current = current_path.as_ref().map(|p| archive::clean_path(p)) == Some(path.clone());
        let is_selected = nav_tree.selected.as_ref().map(|p| archive::clean_path(p)) == Some(path.clone());
        ...
        if is_archive {
            ...
            if resp.clicked() { *open_req = Some(path.clone()); }
        } else {
            ...
            if response.header_response.clicked() {
                ...
                if is_expanded {
                    ...
                    nav_tree.expanded.insert(path.clone());
                }
            }
        }
    }
    ...
    fn rotate_current(&mut self, cw: bool, ctx: &egui::Context) {
        let Some(ref path) = self.navigator.archive_path else { return };
        ...
        for idx in indices {
            if let Some(name) = self.navigator.entries.get(idx).cloned() {
                ...
                self.loader.invalidate(&format!("{}:{}:{}", idx, path.to_string_lossy(), self.navigator.entries[idx]));
            }
        }
        self.schedule_prefetch();
        ctx.request_repaint();
    ...
    fn move_to_dir(&mut self, path: PathBuf, focus_hint: Option<PathBuf>, go_last: bool, ctx: &egui::Context) {
        ...
        self.loader.clear();
        self.error = None;
        self.navigator.move_to_dir(path, focus_hint, go_last, &self.config, self.manga_mode, self.manga_shift);
    ...
    fn navigate_relative_dir(&mut self, forward: bool, ctx: &egui::Context) {
        let Some(curr) = &self.navigator.archive_path else { return };
        if let Some(dest) = self.nav_tree.get_relative_target(curr, forward) {
            let src = curr.clone();
            let go_last = if forward { false } else { self.navigator.open_from_end };
            self.move_to_dir(dest, Some(src), go_last, ctx);
        }
    }
    ...
    fn go_prev(&mut self, ctx: &egui::Context) {
        ...
        let path_str = self.navigator.archive_path.as_ref()
            .map(|p| p.to_string_lossy().to_string()).unwrap_or_default();

        // is_spreadクロージャは、loaderへの不変参照をキャプチャする
        let loader = &self.loader;
        let is_spread = |idx, name: &str| {
            let key = format!("{}:{}:{}", idx, path_str, name);
            loader.get_first_frame(&key).map(|t| t.size_vec2().x > t.size_vec2().y).unwrap_or(false)
        };
        // navigator.go_prevはnavigatorを可変借用する
        if !self.navigator.go_prev(self.manga_mode, self.manga_shift, is_spread) {
    ...
    fn go_next(&mut self, ctx: &egui::Context) {
        ...
        // navigator.go_nextはnavigatorを可変借用する
        if !self.navigator.go_next(self.manga_mode, self.manga_shift, is_spread) {
    ...
    fn open_external(&self) {
        let Some(target_str) = self.navigator.get_current_full_path() else { return };
...
impl eframe::App for App {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        // ── バックグラウンド結果を回収 ──────────────────────────────────
        let failures = self.loader.collect_results(ctx, self.config.linear_filter, &self.navigator.entries);
        for (idx, err) in failures {
            // 今まさに表示しようとしている画像がエラーになったなら表示を切り替える
            if idx == self.navigator.target_index {
                self.error = Some(err);
        ...
        // ── キーボード ──────────────────────────────────────────────────
        let (left, right, up, dn, enter_key, esc_key, t_key, fit_t, zin, zout, manga_t, rcw, rccw, pgup, pgdn, p_key, n_key, s_key, home, end, bs_key, e_key, i_key, alt_pressed, y_key) = ctx.input(|i| (
        ...
        if self.show_tree && !modal_open {
            ...
            let old_selected = self.nav_tree.selected.clone();

            if up { self.nav_tree.move_selection(-1); }
            if dn { self.nav_tree.move_selection(1); }
            if right { self.nav_tree.expand_current(); }
            if left { self.nav_tree.collapse_or_up(); }

            // 選択が変更された場合、プレビューとしてそのパスを開く（一番最初の画像を表示）
            let new_selected = self.nav_tree.selected.clone();
            ...
            if enter_key {
                if let Some(path) = self.nav_tree.activate_current() {
                    self.open_path(path, ctx);
        ...
        if t_key { 
            ...
            if self.show_tree {
                if let Some(path) = &self.navigator.archive_path {
                    ...
                    self.nav_tree.selected = Some(path.clone());
                }
            }
            ctx.request_repaint();
        }
                    ...
                    ui.vertical(|ui| {
                        ...
                        egui::ScrollArea::both()
                            ...
                            .show(ui, |ui| {
                                ...
                                for root in roots {
                                    Self::ui_dir_tree(&mut self.nav_tree, &self.navigator.archive_path, ui, root, ctx, &mut tree_open_req);
                                }
                            });

                        ui.separator();
                        // ステータス表示
                        if let Some(sel) = self.nav_tree.selected.clone() {
                            let count = self.nav_tree.get_image_count(&sel);
                            let name = sel.file_name().map(|f| f.to_string_lossy().to_string())
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            if self.get_texture(self.navigator.current).is_none() {
                ui.centered_and_justified(|ui| {
                    ui.vertical_centered(|ui| {
                        if self.navigator.entries.is_empty() {
                            if self.navigator.archive_path.is_none() {
                            ...
                            let current_file = self.navigator.entries.get(self.navigator.current)
                                .map(|s| s.as_str())
                                .unwrap_or("");
            ...
            let tex1 = self.get_texture_animated(self.navigator.current, ctx);
            ...
            let tex2 = if self.manga_mode && can_pair {
                self.get_texture_animated(self.navigator.current + 1, ctx).and_then(|(t, s)| {
            ...
            egui::ScrollArea::both().show(ui, |ui| {
                ...
                if is_at_end {
                    if let Some(curr) = &self.navigator.archive_path {
                        let siblings = self.nav_tree.get_siblings(curr);
