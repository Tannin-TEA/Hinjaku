...
pub fn send_path_to_existing_instance(path: &Path) -> bool {
    #[cfg(target_os = "windows")]
    {
        ...
        use windows_sys::Win32::Foundation::{GENERIC_WRITE, INVALID_HANDLE_VALUE, CloseHandle, GetLastError, ERROR_PIPE_BUSY, ERROR_FILE_NOT_FOUND};

        let pipe_name: Vec<u16> = "\\\\.\\pipe\\HinjakuIPC\0".encode_utf16().collect();
        unsafe {
            let mut handle = INVALID_HANDLE_VALUE;
            // 受信側が再接続準備中の場合も含め、最大1秒程度リトライして確実に接続を確立する
            for _ in 0..10 {
                handle = CreateFileW(
                    pipe_name.as_ptr(),
                    GENERIC_WRITE,
...
pub fn listen_for_opens(tx: Sender<PathBuf>) {
    #[cfg(target_os = "windows")]
    {
        std::thread::spawn(move || {
            ...
            // windows-sysの機能フラグに左右されず確実に動作させるため定数を再定義
