...
impl App {
    ...
    fn ui_dir_tree(
        ...
        if is_archive {
            ...
            state.set_open(nav_tree.expanded.contains(&path));

            let header_res = state.show_header(ui, |ui| {
                let resp = ui.selectable_label(is_selected, text);
                if is_selected { resp.scroll_to_me(Some(egui::Align::Center)); }
                if resp.clicked() { nav_tree.selected = Some(path.clone()); }
                resp
            });

            if header_res.inner.clicked() {
                if nav_tree.expanded.contains(&path) { nav_tree.expanded.remove(&path); }
                else { nav_tree.expanded.insert(path.clone()); }
            }
    ...
    fn navigate_relative_dir(&mut self, forward: bool, ctx: &egui::Context) {
        ...
        if let Some(pos) = siblings.iter().position(|p| p == curr) {
            let next_pos = if forward { pos + 1 } else { pos.checked_sub(1).unwrap_or(usize::MAX) };
            if next_pos < siblings.len() {
                let dest = siblings[next_pos].clone();
                let src = curr.clone();
                self.move_to_dir(dest, Some(src), if forward { false } else { self.navigator.open_from_end }, ctx);
            }
        }
    }
    ...
    fn go_prev(&mut self, ctx: &egui::Context) {
        ...
        let is_spread = |idx, name: &str| {
            ...
            loader.get_texture(&key).map(|t| t.size_vec2().x > t.size_vec2().y).unwrap_or(false)
        };
...
impl eframe::App for App {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        if self.show_tree && !modal_open {
            let visible = self.nav_tree.get_visible_list();
            let current_idx = self.nav_tree.selected.as_ref().and_then(|p| visible.iter().position(|x| x == p));

            if up {
                if let Some(idx) = current_idx {
                    if idx > 0 { self.nav_tree.selected = Some(visible[idx - 1].clone()); }
                } else if !visible.is_empty() {
                    self.nav_tree.selected = Some(visible[0].clone());
                }
            }
            if dn {
                if let Some(idx) = current_idx {
                    if idx + 1 < visible.len() { self.nav_tree.selected = Some(visible[idx + 1].clone()); }
                } else if !visible.is_empty() {
                    self.nav_tree.selected = Some(visible[0].clone());
                }
            }
            if right {
                if let Some(p) = &self.nav_tree.selected {
                    if p.is_dir() && archive::detect_kind(p) == archive::ArchiveKind::Plain {
                        self.nav_tree.expanded.insert(p.clone());
                    }
                }
            }
            if left {
                if let Some(p) = &self.nav_tree.selected {
                    if self.nav_tree.expanded.contains(p) {
                        self.nav_tree.expanded.remove(p);
                    } else if let Some(parent) = p.parent() {
                        self.nav_tree.selected = Some(parent.to_path_buf());
                    }
                }
            }
            if enter_key {
                if let Some(p) = self.nav_tree.selected.clone() {
                    if archive::detect_kind(&p) != archive::ArchiveKind::Plain {
                        self.open_path(p, ctx);
                        self.show_tree = false;
                    } else if p.is_dir() {
                        if self.nav_tree.expanded.contains(&p) { self.nav_tree.expanded.remove(&p); }
                        else { self.nav_tree.expanded.insert(p); }
                    }
                }
            }
        ...
        if t_key { 
            ...
            if self.show_tree { self.nav_tree.selected = self.navigator.archive_path.clone(); }
        }
        if y_key {
