    ...
    egui::Window::new("キーコンフィグの設定")
        ...
        .show(ctx, |ui| {
            ...
            egui::ScrollArea::vertical().show(ui, |ui| {
                egui::Grid::new("key_config_grid").num_columns(2).striped(true).show(ui, |ui| {
                    // 機能ごとに整理された表示順
                    let display_order = [
                        "PrevPage", "NextPage", "PrevPageSingle", "NextPageSingle", "FirstPage", "LastPage",
                        "PrevDir", "NextDir",
                        "Up", "Down", "Left", "Right", "Enter",
                        "ToggleFullscreen", "ToggleBorderless", "Escape",
                        "ToggleTree", "ToggleFit", "ZoomIn", "ZoomOut",
                        "ToggleManga", "ToggleMangaRtl", "ToggleLinear", "ToggleBg",
                        "RotateCW", "RotateCCW",
                        "SortSettings", "RevealExplorer",
                        "OpenExternal1", "OpenExternal2", "OpenExternal3", "OpenExternal4", "OpenExternal5",
                        "OpenKeyConfig", "Quit"
                    ];

                    let mut shown = HashSet::new();

                    for &key_id in &display_order {
                        if let Some(binding_val) = config.keys.get(key_id) {
                            ui.label(get_action_label(key_id));
                            ...
                            ui.horizontal(|ui| {
                                ...
                                let btn_text = if is_capturing { "録画中…" } else { "⏺" };
                                if ui.selectable_label(is_capturing, btn_text)
                                    .on_hover_text("キーボードのキーを直接入力して設定します")
                                    .clicked() {
                    ...
                    // display_order にない未知のキー（INI直接編集時など）があれば末尾に表示
    ...
    TopBottomPanel::bottom("toolbar").show(ctx, |ui| {
        ui.horizontal(|ui| {
            ...
            if has {
                ...
                let parent_name = manager.archive_path.as_ref()
                ...
                ui.label(format!("{} | {} | {} | {} | {} | [{} {}]", parent_name, short, file_size, res_str, day_str, sort_label, sort_icon));
