...
pub fn load_config_file(custom_name: Option<&str>) -> (Config, Option<PathBuf>) {
    ...
    if let Ok(ini) = Ini::load_from_file(&path) {
        ...
        if let Some(sec) = ini.section(Some("Global") as Option<&str>) {
            ...
            if let Some(v) = sec.get("WindowWidth") { if let Ok(f) = v.parse() { cfg.window_width = f.max(100.0); } }
            if let Some(v) = sec.get("WindowHeight") { if let Ok(f) = v.parse() { cfg.window_height = f.max(100.0); } }
