...
impl App {
    pub fn new(cc: &eframe::CreationContext<'_>, initial_path: Option<PathBuf>, listener: Option<UnixListener>, config_name: Option<String>) -> Self {
        // 日本語フォント
        ...
        // IPCリスナーが渡された場合、ローカル通信待ち受けスレッドを起動
        if let Some(l) = listener {
            let tx = path_tx.clone();
            let ctx = cc.egui_ctx.clone();
            std::thread::spawn(move || {
                while let Ok(mut s) = l.accept() {
                    if let Ok(mut s) = s.0.try_clone() {
    ...
    fn open_path(&mut self, path: PathBuf, ctx: &egui::Context) {
        ...
        if self.manager.tree.nodes.len() > 1000 {
    ...
    fn ui_dir_tree(
        ...
        let cleaned_path = archive::clean_path(&path);
        let is_current = current_path.as_ref().map(|p| archive::clean_path(p)) == Some(cleaned_path.clone());
        let is_selected = nav_tree.selected.as_ref().map(|p| archive::clean_path(p)) == Some(cleaned_path.clone());
        ...
        if is_archive {
            ...
            if resp.clicked() { *open_req = Some(cleaned_path); }
        } else {
            let is_expanded = nav_tree.expanded.contains(&cleaned_path);
            let response = egui::CollapsingHeader::new(text)
                .id_source(&cleaned_path)
                .open(Some(is_expanded))
                .show(ui, |ui| {
                    let children = nav_tree.get_children(&cleaned_path);
                    for p in children {
            ...
            if response.header_response.clicked() {
                nav_tree.selected = Some(cleaned_path.clone());
                if is_expanded {
                    nav_tree.expanded.remove(&cleaned_path);
                } else {
                    nav_tree.expanded.insert(cleaned_path);
                }
            }
        }
    }
...
impl eframe::App for App {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        // ── バックグラウンド結果を回収 ──────────────────────────────────
        let failures = self.manager.update(ctx, self.config.linear_filter);
        for (idx, err) in failures {
            // 今まさに表示しようとしている画像がエラーになったなら表示を切り替える
            if idx == self.manager.target_index {
                self.error = Some(err);
        ...
        if self.is_loading_archive || self.manager.current != self.manager.target_index {
            if self.get_texture(self.manager.target_index).is_some() {
                self.manager.current = self.manager.target_index;
                self.error = None;
                self.is_loading_archive = false;
                self.loading_retry_count = 0;
                self.last_display_change_time = ctx.input(|i| i.time);
            } else if self.loading_retry_count < 3 {
        ...
        if t_key { 
            ...
            if self.show_tree {
                let target = self.manager.archive_path.clone()
                    .or_else(|| archive::get_roots().first().cloned());
                
                if let Some(path) = target {
                    self.manager.tree.expand_to_path(&path);
                    self.manager.tree.selected = Some(path);
                }
            }
            ctx.request_repaint();
        }
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            if self.get_texture(self.manager.current).is_none() {
                ui.centered_and_justified(|ui| {
                    ui.vertical_centered(|ui| {
                        if self.manager.entries.is_empty() {
