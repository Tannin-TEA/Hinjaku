    ...
    fn handle_action(&mut self, ctx: &egui::Context, act: widgets::ViewerAction) {
        match act {
            ...
            widgets::ViewerAction::MoveToCenter => {
                let (mon_w, mon_h) = integrator::get_primary_monitor_size();
                let x = (mon_w - self.config.window_width) / 2.0;
                let y = (mon_h - self.config.window_height) / 2.0;
