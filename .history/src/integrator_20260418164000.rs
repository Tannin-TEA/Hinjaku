...
pub fn launch_external(exe: &str, args_tmpl: &[String], path_p: &str, path_a: &str) -> Result<(), String> {
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        // 実行パス自体の UNCプリフィックスも除去
        let exe_u16: Vec<u16> = format!("{}\0", exe.replace(r"\\?\", "")).encode_utf16().collect();

        // パスを確実に " で囲む（既に囲まれている場合は重ねない）
        let quote = |s: &str| {
            let s = s.trim_matches('"').replace(r"\\?\", "");
            format!("\"{}\"", s)
        };
        ...
        let params_str = if args_tmpl.is_empty() {
            ...
            args_tmpl.iter()
                .map(|arg| {
                    arg.replace("%P", &quote(path_p))
                       .replace("%p", &quote(path_p))
                       .replace("%A", &quote(path_a))
                       .replace("%a", &quote(path_a))
                })
                .collect::<Vec<_>>().join(" ")
        };
...
pub fn reveal_in_explorer(path: &Path) -> Result<(), String> {
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        // エクスプローラは UNCパスを受け付けないため、確実に除去する
        let cleaned = path.to_string_lossy().replace(r"\\?\", "");
