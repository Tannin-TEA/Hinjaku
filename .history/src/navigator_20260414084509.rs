...
impl NavTree {
    ...
    /// 指定したパスの兄弟（同じ階層のターゲット）リストを返す
    pub fn get_siblings(&mut self, path: &std::path::Path) -> Vec<PathBuf> {
        let Some(parent) = path.parent() else { return vec![] };
        if let Some(cached) = self.nodes.get(parent) {
            return cached.clone();
        }
        let targets = archive::list_nav_targets(parent).unwrap_or_default();
        self.nodes.insert(parent.to_path_buf(), targets.clone());
        targets
    }
