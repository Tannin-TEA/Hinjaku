...
pub fn settings_window(
    ...
    egui::Window::new("外部アプリ連携の設定 (送る)")
        ...
        .show(ctx, |ui| {
            ...
            ui.horizontal(|ui| {
                ui.checkbox(&mut config.close_settings_on_apply, "適用後に閉じる");
                ...
                ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                    if ui.button("適用").clicked() {
                        ...
                        saved = true;
                        if config.close_settings_on_apply { *show = false; }
                    }
    ...
    egui::Window::new("キーコンフィグの設定")
        ...
        .show(ctx, |ui| {
            ...
            egui::ScrollArea::vertical().show(ui, |ui| {
                let conflicts = {
                    let mut key_to_actions: std::collections::HashMap<String, Vec<String>> = std::collections::HashMap::new();
                    for (action_id, binding) in &config.keys {
                        for k in binding.split(',') {
                            let k = k.trim();
                            if !k.is_empty() {
                                key_to_actions.entry(k.to_string()).or_default().push(action_id.clone());
                            }
                        }
                    }
                    let mut set = HashSet::new();
                    for (_key_name, actions) in key_to_actions {
                        if actions.len() > 1 {
