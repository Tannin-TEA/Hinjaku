use crate::{archive, integrator, window, config::{self, Config}, manager::{self, Manager}, utils, painter, widgets, input};
...
impl App {
    ...
    fn open_external(&mut self, index: usize, ctx: &egui::Context) {
        let Some(virtual_str) = self.manager.get_current_full_path() else { return };
        let path_v = utils::to_clean_string(std::path::Path::new(&virtual_str)); // %P: 仮想パス

        let path_p = if let Some(base) = self.manager.archive_path.as_ref() {
            if utils::detect_kind(base) == utils::ArchiveKind::Plain {
                // 通常フォルダ内：画像ファイルそのものが物理パス
                let p = if base.is_dir() {
                    let entry = self.manager.entries.get(self.manager.target_index).map(|s| s.as_str()).unwrap_or("");
                    base.join(entry)
                } else {
                    base.clone()
                };
        ...
        if let Err(e) = integrator::launch_external(&app.exe, &app.args, &path_v, &path_p) {
    ...
    fn handle_viewer_keys(&mut self, ctx: &egui::Context, k: &input::KeyboardState) {
        ...
        if k.bs { 
            if let Some(base) = &self.manager.archive_path {
                ...
                if let Err(e) = integrator::reveal_in_explorer(&target) { self.add_toast(e, ctx); }
            }
        }
    ...
    fn execute_mouse_action(&mut self, action_name: &str, ctx: &egui::Context) {
        match action_name {
            "PrevPage" => self.go_prev(ctx), "NextPage" => self.go_next(ctx),
            "PrevPageSingle" => self.go_single_prev(ctx), "NextPageSingle" => self.go_single_next(ctx),
            "PrevDir" => self.go_prev_dir(ctx), "NextDir" => self.go_next_dir(ctx),
            "ToggleFit" => self.handle_action(ctx, widgets::ViewerAction::SetDisplayMode(match self.display_mode { DisplayMode::Fit => DisplayMode::WindowFit, DisplayMode::WindowFit => DisplayMode::Manual, DisplayMode::Manual => DisplayMode::Fit })),
            "ToggleManga" => self.handle_action(ctx, widgets::ViewerAction::ToggleManga),
            _ => {},
        }
    }
    ...
    fn draw_toasts(&mut self, ctx: &egui::Context) {
        ...
        egui::Area::new(egui::Id::new("toasts"))
            ...
            .interactable(false)
            .show(ctx, |ui| {
                ui.set_max_width(300.0); // 最大幅を制限して形状を安定させる
                ui.with_layout(egui::Layout::bottom_up(egui::Align::Max), |ui| {
                    for (m, _) in &self.toasts {
                        egui::Frame::none()
                            .fill(egui::Color32::from_black_alpha(220))
                            .rounding(4.0)
                            .inner_margin(egui::Margin::symmetric(12.0, 8.0))
                            .show(ui, |ui| {
                                ui.set_min_width(260.0); // 幅を一定以上に保ち、形状を安定させる
                                ui.add(egui::Label::new(egui::RichText::new(m).color(egui::Color32::WHITE)).wrap(true));
                            });
                        ui.add_space(4.0);
                    }
                });
    ...
    fn handle_action(&mut self, ctx: &egui::Context, act: widgets::ViewerAction) {
        match act {
            ...
            widgets::ViewerAction::MoveToCenter => window::move_to_center(ctx, self.config.window_width, self.config.window_height),
            widgets::ViewerAction::ToggleWindowCentered => { self.config.window_centered = !self.config.window_centered; if self.config.window_centered { self.applied_initial_center = false; } self.save_config(); }
            widgets::ViewerAction::ResizeWindow(w, h) => { window::request_resize(ctx, w, h); self.config.window_width = w as f32; self.config.window_height = h as f32; self.last_resize_time = ctx.input(|i| i.time); self.save_config(); }
            widgets::ViewerAction::About => self.show_about = true,
            widgets::ViewerAction::SetMouseAction(btn, act) => { if btn == 4 { self.config.mouse4_action = act; } else { self.config.mouse5_action = act; } self.save_config(); }
            _ => {}
        }
    }
}
