...
impl Manager {
    ...
    pub fn open_path(&mut self, path: PathBuf, _config: &Config) {
        ...
        let bp_clone: PathBuf = base_path.clone();
...
pub struct NavTree {
    ...
    archive_reader: Arc<dyn ArchiveReader>, // 追加
}
...
impl NavTree {
    ...
    pub fn get_image_count(&mut self, path: &Path) -> usize {
        ...
        let count = self.archive_reader.list_images(&path).map(|e: Vec<archive::ImageEntry>| e.len()).unwrap_or(0);
        self.image_counts.insert(path, count);
        count
    }
    pub fn expand_to_path(&mut self, path: &Path) { // ... (実装はそのまま)
