...
pub fn sidebar_ui(
    ...
    ui.vertical(|ui| {
            ...
            if let Some(root) = tree_root.clone() {
                if let Some(parent) = root.parent() {
                    if ui.button("⤴ 上へ").on_hover_text("親ディレクトリを起点にする").clicked() {
                        *tree_root = Some(parent.to_path_buf());
                    }
                }
            }
        });
        ...
        ScrollArea::both().max_height(scroll_height).show(ui, |ui| {
            let roots = if let Some(root) = tree_root.clone() { vec![root] } else { nav_tree.get_roots() };
            for root in roots {
