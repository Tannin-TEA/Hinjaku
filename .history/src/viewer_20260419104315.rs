...
impl App {
    ...
    fn update_title(&mut self, ctx: &egui::Context) {
        ...
        ctx.send_viewport_cmd(egui::ViewportCommand::Title(
            format!("Hinjaku - {}{} - {} - [{}]", renderer_str, config_part, folder_name, integrator::get_memory_usage_str())
        ));
    }
        ...
        if self.is_loading_archive || self.manager.current != target {
            ...
            if let Some(tex1) = self.get_texture(target) {
                if self.view.manga_mode {
                    ...
                    let needs_2nd = s1.x <= s1.y && (self.view.manga_shift || target > 0) && target + 1 < self.manager.entries.len();
