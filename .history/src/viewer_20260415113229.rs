...
impl eframe::App for App {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        // ── メインメニューとツールバーの処理 ────────────────────────────
        let menu_action = widgets::main_menu_bar(ctx, &self.config, &self.manager, self.display_mode, self.show_tree, false);
        ...
        if let Some(act) = menu_action.or(toolbar_action) {
            match act {
                ...
                widgets::ViewerAction::RevealInExplorer => { if let Some(path) = &self.manager.archive_path { integrator::reveal_in_explorer(path); } },
                widgets::ViewerAction::OpenExternal => self.open_external(),
                widgets::ViewerAction::OpenExternalSettings => { self.settings_args_tmp = self.config.external_args.join(" "); self.show_settings = true; },
                ...
                widgets::ViewerAction::ToggleManga => { self.manga_mode = !self.manga_mode; self.manager.schedule_prefetch(self.config.linear_filter, self.manga_mode); },
                ...
                widgets::ViewerAction::ToggleLinear => { self.config.linear_filter = !self.config.linear_filter; self.manager.clear_cache(); self.save_config(); self.manager.schedule_prefetch(self.config.linear_filter, self.manga_mode); },
                ...
                widgets::ViewerAction::Seek(idx) => { self.manager.target_index = idx; self.folder_lock_until = 0.0; self.page_lock_until = 0.0; self.manager.schedule_prefetch(self.config.linear_filter, self.manga_mode); },
            }
        }
        ...
        if self.show_tree {
            egui::SidePanel::left("tree_panel").default_width(ctx.screen_rect().width() / 2.0).show(ctx, |ui| {
                // ... (中身は widgets::ui_dir_tree の呼び出し等)
                let roots = self.manager.archive_reader.get_roots();
                for root in roots { widgets::ui_dir_tree(&mut self.manager.tree, &self.manager.archive_path, ui, root, ctx, &mut tree_open_req); }
            });
        }
