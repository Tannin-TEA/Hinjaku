use crate::{archive, config, manager::{self, Manager, Rotation}};
...
impl App {
    ...
    fn get_texture(&self, index: usize) -> Option<&TextureHandle> {
        // index指定の取得もManager側に統合可能だが、CentralPanelでの表示用に簡易実装
        if index == self.manager.current { self.manager.get_current_tex() } else { None }
    }

    /// アニメーション対応のテクスチャ取得。
    fn get_texture_animated(&self, index: usize, ctx: &egui::Context) -> Option<(&TextureHandle, egui::Vec2)> {
        let tex = self.get_texture(index)?;
