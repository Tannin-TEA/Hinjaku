...
impl App {
    ...
    fn update_title(&mut self, ctx: &egui::Context) {
        ...
        let container_name_part = self.manager.archive_path.as_ref()
            .map(|p| format!(" {}", utils::get_display_name(p))).unwrap_or_default();

        ctx.send_viewport_cmd(egui::ViewportCommand::Title(
            format!("Hinjaku - {}{}{} ({})", renderer_str, config_part, container_name_part, integrator::get_memory_usage_str())
        ));
    }
    ...
    fn handle_viewer_keys(&mut self, ctx: &egui::Context, k: &input::KeyboardState) {
        ...
        if k.toggle_debug && ctx.input(|i| i.modifiers.ctrl) {
