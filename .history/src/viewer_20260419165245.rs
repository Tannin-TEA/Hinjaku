...
impl App {
    ...
    fn is_nav_locked(&self, ctx: &egui::Context) -> bool {
        ...
        // 試聴モードがONの場合、表示が追いつくまで操作をロックする
        if self.config.preview_mode && self.manager.current != self.manager.target_index {
            return true;
        }
    ...
    fn handle_viewer_keys(&mut self, ctx: &egui::Context, k: &input::KeyboardState) {
        ...
        // k.toggle_debug は修飾キーがない時のみ true になるため、修飾キー付きは直接判定する
        if ctx.input(|i| i.key_pressed(egui::Key::F12) && i.modifiers.ctrl) {
            // widgets.rs に定義がないため一時コメントアウト
            // self.handle_action(ctx, TogglePreviewMode);
            self.add_toast("試聴モード機能は未実装です（widgets.rsへの定義追加が必要）".to_string(), ctx);
        }
        if k.toggle_tree {
    ...
    fn sync_display_to_target(&mut self, ctx: &egui::Context) {
        ...
        if self.is_loading_archive || self.manager.current != target {
            ...
            if self.get_texture(target).is_some() {
            ...
            if is_ready {
                ...
                self.is_loading_archive = false;
                let now = ctx.input(|i| i.time);
                let guard = if self.config.preview_mode { self.config.preview_guard_duration as f64 } else { ui::PAGE_NAV_GUARD_DURATION };
                self.folder_lock_until = if was_loading { now + ui::FOLDER_NAV_GUARD_DURATION } else { 0.0 };
                self.page_lock_until   = if !was_loading { now + guard } else { 0.0 };
    ...
    fn handle_action(&mut self, ctx: &egui::Context, act: widgets::ViewerAction) {
        ...
        match act {
            ...
            // widgets.rs に定義がないため一時コメントアウト
            // TogglePreviewMode => {
            //     self.config.preview_mode = !self.config.preview_mode;
            //     self.add_toast(format!("試聴モード: {}", if self.config.preview_mode { "ON (制限あり)" } else { "OFF (無制限)" }), ctx);
            //     self.save_config();
            // }
            // SetPreviewGuardDuration(d) => {
            //     self.config.preview_guard_duration = d;
            //     self.save_config();
            // }
