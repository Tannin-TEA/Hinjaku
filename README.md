...
### 1. ナビゲーション・ガード (誤操作防止)
- **フォルダ移動ロック**: `0.1秒`。移動直後の意図しない連打を防止します。
- **ページ移動ロック**: `0.01秒`。マンガモード等の揃え待ち中のガタつきを防止します。
- **ホイールしきい値**: `40.0`。一定量のスクロール蓄積でページをめくります。

### 2. パフォーマンスと制限
- **最大テクスチャサイズ**: `1920px` (Full HD相当)。VRAM 節約のための表示上限です。
- **画像キャッシュ**: 最大 `16枚`。
- **プリフェッチ**: 前方 `3枚`・後方 `2枚` をバックグラウンドで事前読み込み。
- **デコードワーカー**: `4スレッド` で並列処理。
...
| `Renderer` | `Glow` / `Wgpu` | レンダラー（OpenGL / WGPU） |
