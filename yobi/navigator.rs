...
impl NavTree {
    ...
    pub fn get_children(&mut self, dir_path: &std::path::Path) -> Vec<PathBuf> {
        ...
        // メモリ肥大化対策：キャッシュが大きくなりすぎたら一度リセットする
    ...
    /// 指定パス内の画像枚数を取得（キャッシュがあればそれを返す）
    pub fn get_image_count(&mut self, path: &Path) -> usize {
        let path = archive::clean_path(path);
        if let Some(&count) = self.image_counts.get(&path) {
            return count;
        }
        // list_images はディレクトリでもアーカイブでも動作する
    ...
    pub fn collapse_or_up(&mut self) {
        let Some(p) = self.selected.as_ref().map(|p| archive::clean_path(p)) else { return };
        if self.expanded.contains(&p) {
            self.expanded.remove(&p);
        } else if let Some(parent) = p.parent() {
            let parent = archive::clean_path(parent);
    ...
    /// Enter：現在の選択パスを確定して返す
    pub fn activate_current(&mut self) -> Option<PathBuf> {
        // ディレクトリ・アーカイブに関わらず、Enterを押した際の「確定パス」として現在の選択を返す
        self.selected.clone()
    }
